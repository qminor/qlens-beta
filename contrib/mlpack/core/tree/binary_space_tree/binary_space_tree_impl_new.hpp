...
BinarySpaceTree(
    const MatType& data,
    const size_t maxLeafSize, arma::Col<double>* weightvec) :
    left(NULL),
    right(NULL),
    parent(NULL),
    ...
    dataset(new MatType(data)), // Copies the dataset.
	 weights((weightvec != NULL) ? new arma::Col<double>(*weightvec) : NULL)
{
...
template<typename MetricType,
         ...
         template<typename SplitBoundType, typename SplitMatType>
             class SplitType>
BinarySpaceTree<MetricType, StatisticType, MatType, BoundType, SplitType>::
BinarySpaceTree(MatType&& data, const size_t maxLeafSize, arma::Col<double>* weightvec) :
    left(NULL),
    right(NULL),
    parent(NULL),
    begin(0),
    ...
    dataset(new MatType(std::move(data))),
	 weights((weightvec != NULL) ? new arma::Col<double>(std::move(*weightvec)) : NULL)
{
  // Do the actual splitting of this node.
  SplitType<BoundType<MetricType>, MatType> splitter;
  SplitNode(maxLeafSize, splitter);
...
BinarySpaceTree(
    ...
    dataset(&parent->Dataset()), // Point to the parent's dataset.
	 weights((parent->HasWeights()) ? &parent->Weights() : NULL)
{
...
BinarySpaceTree(
    ...
    dataset(&parent->Dataset()),
	 weights((parent->HasWeights()) ? &parent->Weights() : NULL)
{
  if (parent->HasWeights()) weights = &parent->Weights();
  else weights = NULL;
...
BinarySpaceTree(
    ...
    dataset((other.parent == NULL) ? new MatType(*other.dataset) : NULL),
	 weights(((other.parent == NULL) || (!other.HasWeights)) ? NULL : new arma::Col<double>(*other.weights))
{
...
operator=(BinarySpaceTree&& other)
{
  ...
  weights = (other.weights != NULL) ? other.weights : NULL;
    ...
    serialize(Archive& ar, const uint32_t /* version */)
{
  ...
  if (!hasParent)
  {
    ...
    ar(CEREAL_POINTER(datasetTemp));
	 if (weights != NULL) {
		 arma::Col<double>*& weightsTemp = const_cast<arma::Col<double>*&>(weights);
		 ar(CEREAL_POINTER(weightsTemp));
	 }
  }
  ...
  if (!hasParent)
  {
    ...
    while (!stack.empty())
    {
      ...
      stack.pop();
      node->dataset = dataset;
		node->weights = ((weights != NULL) ? weights : NULL);
      if (node->left)
