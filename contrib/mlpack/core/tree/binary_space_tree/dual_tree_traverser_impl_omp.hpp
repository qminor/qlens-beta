    ...
    const size_t level)
{
  ...
  {
    const double rootScore = rule.Score(queryNode, referenceNode, traversalInfo);
            ...
            !queryNode.IsLeaf() && !referenceNode.IsLeaf()))
  {
    ...
    const double leftScore = rule.Score(*queryNode.Left(), referenceNode, leftInfo);
    ++numScores;
    ...
    //rule.TraversalInfo() = traversalInfo;
    const double rightScore = rule.Score(*queryNode.Right(), referenceNode, rightInfo);
    ++numScores;
  ...
  else if (queryNode.IsLeaf() && (!referenceNode.IsLeaf()))
  {
    ...
    double rightScore = rule.Score(queryNode, *referenceNode.Right(), traversalInfo);
    numScores += 2;
  ...
  else
  {
    ...
    typename RuleType::TraversalInfoType leftInfo = traversalInfo;
    typename RuleType::TraversalInfoType rightInfo = traversalInfo;
    double leftScore = rule.Score(*queryNode.Left(), *referenceNode.Left(), leftInfo);
    //rule.TraversalInfo() = traversalInfo;
    double rightScore = rule.Score(*queryNode.Left(), *referenceNode.Right(), rightInfo);
    numScores += 2;
    ...
    if (leftScore < rightScore)
    {
      ...
      if (level % 4 == 0)
      {
			...
			if (rightScore != DBL_MAX)
			{
			  ...
			  Traverse(*queryNode.Left(), *referenceNode.Right(), rightInfo, level+1);
			}
			else
			  ++numPrunes;
			}
    ...
    leftScore = rule.Score(*queryNode.Right(), *referenceNode.Left(), origInfo);
