...
class KMeans
{
 public:
   ...
   */
  bool Cluster(const MatType& data,
  ...
  bool Cluster(const MatType& data,
               ...
               const bool include_weights = false,
               const bool initialGuess = false);
  ...
  */
  ...
  /**
   * Perform k-means clustering on the data (with a weight attached to each data
	* point), returning a list of cluster assignments and also the centroids of
   * each cluster.  Optionally, the vector of assignments can be set to an
   * initial guess of the cluster assignments; to do this, set
   * initialAssignmentGuess to true.  Another way to set initial cluster guesses
   * is to fill the centroids matrix with the centroid guesses, and then set
   * initialCentroidGuess to true.  initialAssignmentGuess supersedes
   * initialCentroidGuess, so if both are set to true, the assignments vector
   * is used.
   *
   * @tparam MatType Type of matrix (arma::mat or arma::sp_mat).
   * @param data Dataset to cluster.
