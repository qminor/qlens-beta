...
struct LensIntegral 
{
	...
	QScalar fourier_kappa_m(const QScalar r, const QScalar phi, const int mval_in, const QScalar fourier_ival_in);
};
...
template <typename QScalar>
QScalar LensIntegral<QScalar>::fourier_kappa_m(const QScalar r, const QScalar phi, const int mval_in, const QScalar fourier_ival_in)
{
	QScalar ans, mphi;
	mval = mval_in;
	fourier_ival = fourier_ival_in;
...
#ifdef USE_STAN
	bool output = false;
	if constexpr (std::is_same_v<QScalar, stan::math::var>) output = true;
#endif

	int order, j;
#ifdef USE_STAN
	if (output) std::cout << "HARG0" << std::endl;
#endif
	do {
		weightptr = Patterson::pat_weights[level];
#ifdef USE_STAN
	if (output) std::cout << "HARG1" << std::endl;
#endif
		result_old = result;
		order = Patterson::pat_orders[level];
#ifdef USE_STAN
	if (output) std::cout << "HARG2" << std::endl;
#endif
		istep = 512 / (order+1);
		istart = istep - 1;
		istep *= 2;
...
#ifdef USE_STAN
	if (output) std::cout << "HARG3" << std::endl;
#endif
	QScalar herg = absum + abdif*Patterson::pat_points[i];
#ifdef USE_STAN
	if (output) std::cout << "HARG3b" << std::endl;
#endif
			pat_funcs[i] = (this->*func)(herg);
			//pat_funcs[i] = (this->*func)(absum + abdif*Patterson::pat_points[i]);
#ifdef USE_STAN
	if (output) std::cout << "HARG4" << std::endl;
#endif
...
#ifdef USE_STAN
	if (output) std::cout << "HARG5" << std::endl;
#endif
...
#ifdef USE_STAN
	if (output) std::cout << "HARG6" << std::endl;
#endif
