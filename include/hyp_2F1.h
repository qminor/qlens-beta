...
complex<QScalar> Gamma_ratio_diff_small_eps (const complex<QScalar> &z,const complex<QScalar> &eps);
template <typename QScalar>
complex<QScalar> Gamma_inv_diff_eps (const complex<QScalar> &z,const complex<QScalar> &eps);
template <typename QScalar>
complex<QScalar> A_sum_init (const int m,const complex<QScalar> &eps,const complex<QScalar> &Gamma_inv_one_meps);
template <typename QScalar>
complex<QScalar> log_A_sum_init (const int m,const complex<QScalar> &eps);
template <typename QScalar>
complex<QScalar> B_sum_init_PS_one (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c, const complex<QScalar> &Gamma_c,const complex<QScalar> &Gamma_inv_one_meps, const complex<QScalar> &Gamma_inv_eps_pa_pm,const complex<QScalar> &Gamma_inv_eps_pb_pm, const complex<QScalar> &one_minus_z,const int m,const complex<QScalar> &eps);

template <typename QScalar>
complex<QScalar> B_sum_init_PS_infinity (const complex<QScalar> &a,const complex<QScalar> &c, const complex<QScalar> &Gamma_c,const complex<QScalar> &Gamma_inv_cma, const complex<QScalar> &Gamma_inv_one_meps,const complex<QScalar> &Gamma_inv_eps_pa_pm, const complex<QScalar> &z,const int m,const complex<QScalar> &eps);

template <typename QScalar>
void cv_poly_der_tab_calc (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z,QScalar cv_poly_der_tab[]);
template <typename QScalar>
QScalar cv_poly_der_calc (const QScalar cv_poly_der_tab[],const QScalar x);
template <typename QScalar>
int min_n_calc (const QScalar cv_poly_der_tab[]);
template <typename QScalar>
complex<QScalar> hyp_PS_zero (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z);
template <typename QScalar>
complex<QScalar> hyp_PS_one (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &one_minus_z);
template <typename QScalar>
complex<QScalar> hyp_PS_infinity (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z);
template <typename QScalar>
complex<QScalar> hyp_PS_complex_plane_rest (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z);
template <typename QScalar>
complex<QScalar> hyp_2F1 (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z);
template <typename QScalar>
QScalar test_2F1 (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z,const complex<QScalar> &F);
...
// g,c[15]: QScalar and table of 15 QScalars defining the Lanczos sum so that it provides the Gamma function precise up to machine accuracy.
...
complex<QScalar> Gamma_ratio_diff_small_eps (const complex<QScalar> &z,const complex<QScalar> &eps)
{
  const QScalar g = 4.7421875;
  if (inf_norm (eps) > 0.1) cout<<"One must have |eps|oo < 0.1 in Gamma_ratio_diff_small_eps.", abort ();

  const complex<QScalar> eps_pz = z + eps,z_m_0p5 = z - 0.5,z_pg_m0p5 = z_m_0p5 + g,eps_pz_pg_m0p5 = z_pg_m0p5 + eps, zm1 = z - 1.0,zm1_p_eps = zm1 + eps;
  const QScalar x = real (z),eps_px = real (eps_pz);
  const int n = cast_int (my_rint (x)),m = cast_int (my_rint (eps_px));
  ...
  if ((x >= 0.5) || (eps_px >= 0.5))
  {
    complex<QScalar> sum_num = 0.0, sum_den = c[0];

    for (int i = 1 ; i < 15 ; i++) 
    {
      const complex<QScalar> ci_zm1_pi_inv = c[i]/(zm1 + i);
      sum_num += ci_zm1_pi_inv/(zm1_p_eps + i), sum_den += ci_zm1_pi_inv;
    }

    if (eps != 0.0)
      return expm1 (z_m_0p5*log1p (eps/z_pg_m0p5) + eps*complog (eps_pz_pg_m0p5) - eps + log1p (-eps*sum_num/sum_den))/eps;
    else
      return (z_m_0p5/z_pg_m0p5 + complog (eps_pz_pg_m0p5) - 1.0 - sum_num/sum_den);
  }
  else
  {
    if (eps != 0.0)
    {
      const complex<QScalar> Pi_eps = M_PI*eps,term = sin (Pi_eps)/tan (M_PI*(z-n)),T1_eps_z = (cos (Pi_eps) + term)*Gamma_ratio_diff_small_eps<QScalar> (1.0 - z,-eps);
      const complex<QScalar> sin_Pi_2_eps = sin (M_PI_2*eps),T2_eps_z = (2.0*sin_Pi_2_eps*sin_Pi_2_eps - term)/eps, T_eps_z = T1_eps_z + T2_eps_z;
      
      return (T_eps_z/(1.0 - eps*T_eps_z));
    }
    else
      return (Gamma_ratio_diff_small_eps<QScalar> (1.0 - z,0.0) - M_PI/tan (M_PI*(z-n)));
  }
}
...
complex<QScalar> Gamma_inv_diff_eps (const complex<QScalar> &z,const complex<QScalar> &eps)
{
  const complex<QScalar> eps_pz = z + eps;
  const QScalar x = real (z),eps_px = real (eps_pz);
  const int n = cast_int (my_rint (x)), m = cast_int (my_rint (eps_px));
  ...
  { 
    ...
    {
      const QScalar z_neg_int_distance = inf_norm (z + abs (n)), eps_pz_neg_int_distance = inf_norm (eps_pz + abs (m));

      if (z_neg_int_distance < eps_pz_neg_int_distance)
	return Gamma_ratio_diff_small_eps<QScalar> (z,eps)*Gamma_inv (eps_pz);
      else
	return Gamma_ratio_diff_small_eps<QScalar> (eps_pz,-eps)*Gamma_inv (z);
    }
  }
...
complex<QScalar> A_sum_init (const int m,const complex<QScalar> &eps,const complex<QScalar> &Gamma_inv_one_meps)
{
  const complex<QScalar> one_meps = 1.0 - eps;

  if (one_meps-m != 1-m)
  {
    complex<QScalar> Gamma_inv_one_meps_mm = Gamma_inv_one_meps;
...
complex<QScalar> log_A_sum_init (const int m,const complex<QScalar> &eps)
{
  const complex<QScalar> one_meps_mm = 1.0 - m - eps;

  if (one_meps_mm != 1-m)
    return (-log_Gamma (one_meps_mm) - complog (eps));
  else
  {
    const complex<QScalar> i_Pi(0,M_PI);

    QScalar log_fact = 0.0;
    for (int n = 2 ; n < m ; n++) log_fact += log (static_cast<QScalar>(n));
...
template <typename QScalar>
complex<QScalar> B_sum_init_PS_one (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,
				   const complex<QScalar> &Gamma_c,const complex<QScalar> &Gamma_inv_one_meps,
				   const complex<QScalar> &Gamma_inv_eps_pa_pm,const complex<QScalar> &Gamma_inv_eps_pb_pm,
				   const complex<QScalar> &one_minus_z,const int m,const complex<QScalar> &eps)
{
  const QScalar inf_norm_eps = inf_norm (eps),phase = (m%2 == 0) ? (1) : (-1);
  const complex<QScalar> a_pm = a + m,b_pm = b + m,one_meps = 1.0 - eps,Pi_eps = M_PI*eps,Pi_eps_pm = M_PI*(eps + m);

  complex<QScalar> Gamma_inv_one_meps_mm = Gamma_inv_one_meps;
  for (int i = 1 ; i <= m ; i++) Gamma_inv_one_meps_mm *= one_meps - i;
  if (inf_norm_eps > 0.1)
  {
    const complex<QScalar> Gamma_inv_eps_pm_p1 = phase*sin (Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm);
    complex<QScalar> prod1 = Gamma_inv_one_meps*Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm;
    for (int n = 0 ; n < m ; n++) prod1 *= (a + n)*(b + n)/(n + 1.0);
    const complex<QScalar> prod2 = Gamma_inv (a)*Gamma_inv (b)*Gamma_inv_eps_pm_p1*comppow (one_minus_z,eps),res = Gamma_c*(prod1 - prod2)/eps;
    return res;
  }
  else
  {
    QScalar Gamma_inv_mp1 = 1.0;
    complex<QScalar> prod_ab = 1.0;
    for (int n = 0 ; n < m ; n++) Gamma_inv_mp1 /= n + 1.0, prod_ab *= (a + n)*(b + n);

    const bool is_eps_non_zero = (one_meps-m != 1-m);
    const complex<QScalar> Gamma_inv_eps_pm_p1 = (is_eps_non_zero) ? (phase*sin (Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm)) : (Gamma_inv_mp1);
    const complex<QScalar> Gamma_inv_a_pm = Gamma_inv (a_pm),Gamma_inv_b_pm = Gamma_inv (b_pm);
    const complex<QScalar> z_term = (is_eps_non_zero) ? (expm1 (eps*complog (one_minus_z))/eps) : (complog (one_minus_z));

    const complex<QScalar> prod1 = Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm*(Gamma_inv_mp1*Gamma_inv_diff_eps<QScalar> (1.0,-eps) + Gamma_inv_diff_eps<QScalar> (m+1,eps));
    const complex<QScalar> prod2 = Gamma_inv_eps_pm_p1*(Gamma_inv_eps_pb_pm*Gamma_inv_diff_eps<QScalar> (a_pm,eps) + Gamma_inv_a_pm*Gamma_inv_diff_eps<QScalar> (b_pm,eps));
    ...
    else
    {
      ...
      complex<QScalar> prod1 = Gamma_inv_one_meps*Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm;
      for (int n = 0 ; n < m ; n++) prod1 *= (a + n)*(b + n)/(n + 1.0);
      const complex<QScalar> prod2 = Gamma_inv (a)*Gamma_inv (b)*Gamma_inv_eps_pm_p1*comppow (one_minus_z,eps),res_default = Gamma_c*(prod1 - prod2)/eps;
      return res_default;
    }
  }
}
...
template <typename QScalar>
complex<QScalar> B_sum_init_PS_infinity (const complex<QScalar> &a,const complex<QScalar> &c,
					const complex<QScalar> &Gamma_c,const complex<QScalar> &Gamma_inv_cma,
					const complex<QScalar> &Gamma_inv_one_meps,const complex<QScalar> &Gamma_inv_eps_pa_pm,
					const complex<QScalar> &z,const int m,const complex<QScalar> &eps)
{
  const QScalar inf_norm_eps = inf_norm (eps),phase = (m%2 == 0) ? (1) : (-1);
  const complex<QScalar> cma = c - a,a_mc_p1 = 1.0 - c + a,a_mc_p1_pm = a_mc_p1 + m,cma_meps = cma - eps,eps_pa_mc_p1 = eps + a_mc_p1,a_pm = a + m;
  const complex<QScalar> Gamma_inv_cma_meps = Gamma_inv (cma_meps),one_meps = 1.0 - eps,Pi_eps = M_PI*eps,Pi_eps_pm = M_PI*(eps + m);
  ...
  { 
    const complex<QScalar> Gamma_inv_eps_pm_p1 = phase*sin (Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm);
    complex<QScalar> prod1 = Gamma_inv_cma*Gamma_inv_eps_pa_pm*Gamma_inv_one_meps,prod2 = Gamma_inv (a)*Gamma_inv_cma_meps*Gamma_inv_eps_pm_p1*comppow (-z,-eps);

    for (int n = 0 ; n < m ; n++)
    {
      const QScalar n_p1 = n + 1.0;
      const complex<QScalar> a_pn = a + n,a_mc_p1_pn = a_mc_p1 + n,eps_pa_mc_p1_pn = eps_pa_mc_p1 + n;
      prod1 *= a_pn*a_mc_p1_pn/n_p1, prod2 *= eps_pa_mc_p1_pn;
    }

    const complex<QScalar> res = Gamma_c*(prod1 - prod2)/eps;
    return res;
  }
  else
  {
    const int n0 = -cast_int (my_rint (real (a_mc_p1)));
    const bool is_eps_non_zero = (one_meps-m != 1-m),is_n0_here = (n0 >= 0) && (n0 < m);

    QScalar Gamma_inv_mp1 = 1.0;
    complex<QScalar> prod_a = 1.0,prod_a_mc_p1 = 1.0,prod_eps_pa_mc_p1_n0 = (is_n0_here) ? (1.0) : (0.0),prod_eps_pa_mc_p1 = 1.0,sum = 0.0;
    for (int n = 0 ; n < m ; n++)
    {
      const complex<QScalar> a_pn = a + n,a_mc_p1_pn = a_mc_p1 + n,eps_pa_mc_p1_pn = eps_pa_mc_p1 + n;
    ...
    const complex<QScalar> Gamma_inv_eps_pm_p1 = (is_eps_non_zero) ? (phase*sin (Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm)) : (Gamma_inv_mp1);
    const complex<QScalar> sum_term = (is_eps_non_zero) ? (expm1 (sum)/eps) : (sum),prod_diff_eps = prod_eps_pa_mc_p1_n0 + prod_a_mc_p1*sum_term;

    const complex<QScalar> z_term = (is_eps_non_zero) ? (expm1 (-eps*complog (-z))/eps) : (-complog (-z));
    const complex<QScalar> Gamma_inv_a_pm = Gamma_inv (a_pm),Gamma_prod1 = Gamma_inv_cma*Gamma_inv_eps_pa_pm;
    const complex<QScalar> prod1 = Gamma_prod1*Gamma_inv_mp1*(Gamma_inv_diff_eps<QScalar> (1.0,-eps)*prod_eps_pa_mc_p1 - Gamma_inv_one_meps*prod_diff_eps);
    const complex<QScalar> prod_2a = Gamma_prod1*Gamma_inv_diff_eps<QScalar> (m+1,eps),prod_2b = Gamma_inv_cma*Gamma_inv_eps_pm_p1*Gamma_inv_diff_eps<QScalar> (a_pm,eps);
	...
	prod1 *= a_pn*a_mc_p1_pn/n_p1, prod2 *= eps_pa_mc_p1_pn;
      ...
      const complex<QScalar> res_default = Gamma_c*(prod1 - prod2)/eps;
      return res_default;
    }
  }
}
...
// ----------
// a,b,c,z: a,b,c and z parameters and arguments of the 2F1(a,b,c,z) function.
// cv_poly_der_tab[3]: table of four QScalars containing the P'(X) components.
// mod_a2,mod_b2,mod_c2,mod_z2,R_a,Re_b,Re_c: |a|^2, |b|^2, |c|^2, |z|^2, Re(a), Re(b), Re(c), with which P(X) can be expressed.

template <typename QScalar>
void cv_poly_der_tab_calc (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z,QScalar cv_poly_der_tab[])
{
  const QScalar mod_a2 = norm (a),mod_b2 = norm (b),mod_c2 = norm (c),mod_z2 = norm (z);
  const QScalar Re_a = real (a),Re_b = real (b),Re_c = real (c);
...
QScalar cv_poly_der_calc (const QScalar cv_poly_der_tab[],const QScalar x)
{
  const QScalar Px = cv_poly_der_tab[0] + x*(cv_poly_der_tab[1] + x*(cv_poly_der_tab[2] + x*cv_poly_der_tab[3]));
...
// Variables
// ---------
// cv_poly_der_tab: table of four QScalars containing the P'(X) coefficients
...
int min_n_calc (const QScalar cv_poly_der_tab[])
{
  const QScalar C1 = cv_poly_der_tab[1],C2 = cv_poly_der_tab[2],three_C3 = 3.0*cv_poly_der_tab[3];
  const QScalar Delta = C2*C2 - three_C3*C1;
  ...
  else
  {
    ...
    if constexpr (std::is_same_v<QScalar, stan::math::var>)
	     largest_root = -(C2.val() + sqrt (Delta.val()))/three_C3.val();
	 else
#endif
    largest_root = -(C2 + sqrt (Delta))/three_C3;
    return max (cast_int (ceil (largest_root)),0);
  }
}
...
complex<QScalar> hyp_PS_zero (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z)
{  
  complex<QScalar> term = 1.0,sum = 1.0;
  
  const int na = abs (cast_int (my_rint (real (a)))),nb = abs (cast_int (my_rint (real (b))));
  ...
  else
  {
    ...
    while (possible_false_cv || (inf_norm (term) > 1E-15))
    {
      term *= z*(a+n)*(b+n)/((n+1.0)*(c+n));
      sum += term;
      if (possible_false_cv && (n >= min_n)) possible_false_cv = (cv_poly_der_calc<QScalar> (cv_poly_der_tab,n) > 0);
      n++;
    }
  }
...
complex<QScalar> hyp_PS_one (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &one_minus_z)
{
  const int m = cast_int (my_rint (real (c - a - b))),phase = (m%2 == 0) ? (1) : (-1),m_m1 = m - 1, m_p1 = m + 1;
  const complex<QScalar> eps = c - a - b - m,eps_pm = eps + m,eps_pm_p1 = eps_pm + 1.0,a_pm = a + m,b_pm = b + m,one_meps = 1.0 - eps,one_meps_mm = one_meps - m;
  const complex<QScalar> eps_pa = a + eps,eps_pb = b + eps,eps_pa_pm = eps_pa + m,eps_pb_pm = eps_pb + m,Pi_eps = M_PI*eps,Gamma_c = 1.0/Gamma_inv (c);
  const complex<QScalar> Gamma_inv_eps_pa_pm = Gamma_inv (eps_pa_pm),Gamma_inv_eps_pb_pm = Gamma_inv (eps_pb_pm);
  const complex<QScalar> Gamma_prod = Gamma_c*Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm,Gamma_inv_one_meps = Gamma_inv (one_meps);
  ...
  const complex<QScalar> mcom = m;
  const complex<QScalar> pow_mzp1_m = comppow (one_minus_z,mcom);
  const complex<QScalar> B_first_term = B_sum_init_PS_one (a,b,c,Gamma_c,Gamma_inv_one_meps,Gamma_inv_eps_pa_pm,Gamma_inv_eps_pb_pm,one_minus_z,m,eps)*pow_mzp1_m;

  complex<QScalar> prod_B = pow_mzp1_m;
  for (int n = 0 ; n < m_m1 ; n++) 
  {
    const complex<QScalar> ratio = (a + n)*(b + n)/(n + 1.0);
    A_term *= one_minus_z*ratio/(n + one_meps_mm), A_sum += A_term, prod_B *= ratio;
  }

  if (m > 0) prod_B *= (a + m - 1.0)*(b + m - 1.0)/m;

  complex<QScalar> B_extra_term = prod_B*Gamma_prod*Gamma_inv_one_meps,B_term = B_first_term,B_sum = B_first_term;
  const QScalar B_prec = 1E-15*inf_norm (B_first_term);

  QScalar cv_poly1_der_tab[4],cv_poly2_der_tab[4];
  ...
  while (possible_false_cv || (inf_norm (B_term) > B_prec))
  {
    ...
    const complex<QScalar> a_pm_pn = a_pm + n,b_pm_pn = b_pm + n,eps_pm_p1_pn = eps_pm_p1 + n,n_p1_meps = one_meps + n;
    const complex<QScalar> eps_pa_pm_pn = eps_pa_pm + n,eps_pb_pm_pn = eps_pb_pm + n,eps_pm_pn = eps_pm + n;
    const complex<QScalar> prod1 = eps_pa_pm_pn*eps_pb_pm_pn,prod2 = eps_pm_p1_pn*n_p1,prod3 = a_pm_pn*b_pm_pn;
    ...
    if (possible_false_cv && (n >= min_n)) possible_false_cv = (cv_poly_der_calc<QScalar> (cv_poly1_der_tab,n) > 0) || (cv_poly_der_calc<QScalar> (cv_poly2_der_tab,n) > 0);
    n++;
  }
...
complex<QScalar> hyp_PS_infinity (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z)
{
  const int m = cast_int (my_rint (real (b - a))),phase = (m%2 == 0) ? (1) : (-1), m_m1 = m - 1,m_p1 = m + 1;
  const complex<QScalar> eps = b - a - m,a_mc_p1 = 1.0 - c + a,one_meps = 1.0 - eps,one_meps_mm = one_meps - m,a_pm = a + m,a_mc_p1_pm = a_mc_p1 + m,cma = c - a;
  const complex<QScalar> eps_pa = eps + a, eps_pm_p1 =  eps + m + 1,eps_pa_mc_p1_pm = eps + a_mc_p1_pm,Pi_eps = M_PI*eps;
  const complex<QScalar> eps_pa_pm = eps_pa + m,eps_pm = eps + m,Gamma_c = 1.0/Gamma_inv (c),Gamma_inv_eps_pa_pm = Gamma_inv (eps_pa_pm);
  const complex<QScalar> Gamma_inv_cma = Gamma_inv (cma),z_inv = 1.0/z,pow_mz_ma = comppow (-z,-a),Gamma_inv_one_meps = Gamma_inv (one_meps);
  const complex<QScalar> Gamma_prod = Gamma_c*Gamma_inv_cma*Gamma_inv_eps_pa_pm;
  ...
  const complex<QScalar> mcom = m;
  const complex<QScalar> pow_z_inv_m = comppow (z_inv,mcom);
  const complex<QScalar> B_first_term = B_sum_init_PS_infinity (a,c,Gamma_c,Gamma_inv_cma,Gamma_inv_one_meps,Gamma_inv_eps_pa_pm,z,m,eps)*pow_z_inv_m;

  complex<QScalar> prod_B = pow_z_inv_m;
  for (int n = 0 ; n < m_m1 ; n++) 
  {
    const complex<QScalar> ratio = (a + n)*(a_mc_p1 + n)/(n + 1.0);
  ...
  {
    const int n_pm_p1 = n + m_p1,n_p1 = n + 1;
    const complex<QScalar> a_pm_pn = a_pm + n,a_mc_p1_pm_pn = a_mc_p1_pm + n,eps_pm_p1_pn = eps_pm_p1 + n,n_p1_meps = one_meps + n;
    const complex<QScalar> eps_pa_pm_pn = eps_pa_pm + n,eps_pa_mc_p1_pm_pn = eps_pa_mc_p1_pm + n,eps_pm_pn = eps_pm + n;
    const complex<QScalar> prod1 = eps_pa_pm_pn*eps_pa_mc_p1_pm_pn,prod2 = eps_pm_p1_pn*n_p1,prod3 = a_pm_pn*a_mc_p1_pm_pn;
...
complex<QScalar> hyp_PS_complex_plane_rest (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z)
{
  const QScalar abs_z = abs (z);
  const bool is_abs_z_small = abs_z < 1.0;
  
  const complex<QScalar> z0 = (is_abs_z_small) ? (0.9*z/abs_z) : (1.1*z/abs_z),zc = z - z0;
  const complex<QScalar> zc_z0_ratio = zc/(z0*(1.0 - z0)),z0_term1 = 2.0*z0 - 1.0,z0_term2 = c - (a + b + 1.0)*z0;
  
  const complex<QScalar> hyp_PS_z0 = (is_abs_z_small) ? (hyp_PS_zero (a,b,c,z0)) : (hyp_PS_infinity (a,b,c,z0));
  const complex<QScalar> dhyp_PS_z0 = (is_abs_z_small) ? (hyp_PS_zero (a + 1.0,b + 1.0,c + 1.0,z0)*a*b/c) : (hyp_PS_infinity (a + 1.0,b + 1.0,c + 1.0,z0)*a*b/c);

  int n = 0;
  complex<QScalar> an = hyp_PS_z0, anp1 = zc*dhyp_PS_z0, sum = an + anp1;

  const QScalar prec = 1E-15*(inf_norm (an) + inf_norm (anp1));

  while (inf_norm (an) + inf_norm (anp1) > prec)
  {
    const complex<QScalar> anp2 = zc_z0_ratio*(anp1*(n*z0_term1 - z0_term2) + an*zc*(a + n)*(b + n)/(n + 1))/(n + 2);
    sum += anp2;
    n++;
    an = anp1;
    anp1 = anp2;
...
complex<QScalar> hyp_2F1 (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z)
{
  ...
  const int na = cast_int (my_rint (Re_a)),nb = cast_int (my_rint (Re_b)),nc = cast_int (my_rint (Re_c));
  const bool is_a_neg_int = (a == na) && (na <= 0),is_b_neg_int = (b == nb) && (nb <= 0),is_c_neg_int = (c == nc) && (nc <= 0);
  const complex<QScalar> zm1 = z-1.0;

  if (is_c_neg_int)
  {
    if (is_a_neg_int && (nc < na))
    {
      const complex<QScalar> z_over_zm1 = z/zm1;
      return ((z == 1.0) || (abs (z) < abs (z_over_zm1))) ? (hyp_PS_zero (a,b,c,z)) : (comppow (-zm1,-a)*hyp_PS_zero (a,c-b,c,z_over_zm1));
    }
    else if (is_b_neg_int && (nc < nb))
    {
      const complex<QScalar> z_over_zm1 = z/zm1;
      return ((z == 1.0) || (abs (z) < abs (z_over_zm1))) ? (hyp_PS_zero (a,b,c,z)) : (comppow (-zm1,-b)*hyp_PS_zero (b,c-a,c,z_over_zm1));
    }
    else 
  ...
  const complex<QScalar> z_shift(real (z),-1E-307);
  ...
  if (!ab_condition || !cab_condition)
  {
    ...
    else if (!cab_condition && ab_condition) return comppow (-zm1,c-a-b)*hyp_2F1 (c-b,c-a,c,z);
    else return comppow (-zm1,c-a-b)*hyp_2F1 (c-a,c-b,c,z);
  }

  const QScalar abs_zm1 = abs (zm1);
  if (abs_zm1 < 1E-5) return hyp_PS_one (a,b,c,-zm1);

  const QScalar abs_z = abs (z),abs_z_inv = 1.0/abs_z,abs_z_over_zm1 = abs_z/abs_zm1,abs_zm1_inv = 1.0/abs_zm1,abs_zm1_over_z = 1.0/abs_z_over_zm1;
  ...
  const QScalar R_tab[5] = {0.5,0.6,0.7,0.8,0.9};
  
  for (unsigned int i = 0 ; i < 5 ; i++)
  {
    const QScalar R = R_tab[i];

    if (abs_z <= R) return hyp_PS_zero (a,b,c,z);
    if (is_cmb_small && (abs_z_over_zm1 <= R)) return comppow (-zm1,-a)*hyp_PS_zero (a,c-b,c,z/zm1);
  }
  ...
  for (unsigned int i = 0 ; i < 5 ; i++)
  {
    ...
    if (is_cmb_small && (abs_zm1_over_z <= R)) return comppow (-zm1,-a)*hyp_PS_infinity (a,c-b,c,z/zm1);

    if (are_abc_small && (abs_zm1 <= R)) return hyp_PS_one (a,b,c,-zm1);
    if (are_a_cmb_c_small && (abs_zm1_inv <= R)) return comppow (-zm1,-a)*hyp_PS_one (a,c-b,c,-1.0/zm1);
  }
...
QScalar test_2F1 (const complex<QScalar> &a,const complex<QScalar> &b,const complex<QScalar> &c,const complex<QScalar> &z,const complex<QScalar> &F)
{
  ...
  {
    const complex<QScalar> dF = hyp_2F1(a+1.0,b+1.0,c+1.0,z)*a*b/c;
  ...
  {
    const complex<QScalar> dF = hyp_2F1(a+1.0,b+1.0,c+1.0,z)*a*b/c;
    const complex<QScalar> d2F = hyp_2F1(a+2.0,b+2.0,c+2.0,z)*a*(a+1.0)*b*(b+1.0)/(c*(c+1.0));
