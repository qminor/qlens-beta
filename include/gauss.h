...
// NOTE: when you create a GaussLegendre object, initialize as "GaussLegendre<std::function<double(double)>> gauss_object"; likewise for GaussPatterson, ClenshawCurtis
template <typename Func, typename T>
class GaussLegendre 
{
     public:
         inline static double *weights = NULL;
         inline static double *points = NULL;
         inline static int numberOfPoints = 60;
     protected:
			inline static const double EPS = 1e-6;
			inline static const double RT4M_PI_INV = 0.7511255444649425;
     public:
			GaussLegendre() {}
			GaussLegendre(const int N) { SetGaussLegendre(N); }
			static void allocate_quadrature_tables(const int N);
			static void deallocate_quadrature_tables();
			static void SetGaussLegendre(const int N)
			{
				if (numberOfPoints != N) allocate_quadrature_tables(N);
			}
			T NIntegrate(Func func, T, T);
			T NIntegrateInf(Func func);
};
...
{
		public:
         inline static double pat_points[512];
         inline static double **pat_weights = NULL;
			inline static int *pat_orders = NULL;
     ...
     public:
         ...
         ~GaussPatterson() {}
         T AdaptiveQuad(Func func, T, T, bool& converged, bool outer = false);
};
...
{
		public:
         inline static double **cc_weights = NULL;
         inline static double *cc_points = NULL;
			inline static int *cc_lvals = NULL;
         inline static int cc_N = 0;
			inline static int cc_nlevels = 0;

			double cc_tolerance, cc_tolerance_outer;
			bool include_endpoints; // if set to "false", use Fejer's quadature rule (type 2) which excludes endpoints
			bool show_convergence_warning;
     ...
     public:
          ...
          ~ClenshawCurtis();
          T AdaptiveQuadCC(Func func, T, T, bool& converged, bool outer = false);
};


template <typename Func, typename T>
void GaussLegendre<Func,T>::allocate_quadrature_tables(const int N)
{
	numberOfPoints = N;
...
template <typename Func, typename T>
void GaussLegendre<Func,T>::deallocate_quadrature_tables()
{
	if (points != NULL) delete[] points;
	if (weights != NULL) delete[] weights;
}
...
template <typename Func, typename T>
T GaussLegendre<Func,T>::NIntegrate(Func func, T a, T b)
{
	T result = 0;

	for (int i = 0; i < numberOfPoints; i++)
		result += weights[i]*func(((a+b) + (b-a)*points[i])/2.0);

	return (b-a)*result/2.0;
}

template <typename Func, typename T>
T GaussLegendre<Func,T>::NIntegrateInf(Func func)
{
	T result = 0;

	for (int i = 0; i < numberOfPoints; i++)
	    result += 2.0/(1.0-points[i])/(1.0-points[i])*weights[i]*Func((1.0+points[i])/(1.0-points[i]));
	
	return result;
}

template <typename Func, typename T>
ClenshawCurtis<Func,T>::ClenshawCurtis()
{
	if (cc_N > 0) {
		cc_funcs = new T[cc_N];
		cc_funcs2 = new T[cc_N];
	} else {
		cc_funcs = NULL;
		cc_funcs2 = NULL;
	}
...
template <typename Func, typename T>
void ClenshawCurtis<Func,T>::SetClenshawCurtis(const int nlevels_in, const double tol_in, const bool include_endpoints_in, const bool show_warnings)
{
	...
	cc_tolerance_outer = tol_in;
	show_convergence_warning = show_warnings;
	if ((nlevels_in != cc_nlevels) or (include_endpoints_in != include_endpoints)) {
		include_endpoints = include_endpoints_in;
		allocate_cc_quadrature_tables(nlevels_in,include_endpoints_in);
		if (cc_funcs != NULL) {
			delete[] cc_funcs;
			delete[] cc_funcs2;
		}
...
template <typename Func, typename T>
void ClenshawCurtis<Func,T>::allocate_cc_quadrature_tables(const int nlevels_in, const bool include_endpoints_in)
{
	if (cc_points != NULL) {
		delete[] cc_points;
		delete[] cc_lvals;
...
template <typename Func, typename T>
void ClenshawCurtis<Func,T>::deallocate_cc_quadrature_tables()
{
	if (cc_points != NULL) {
		delete[] cc_points;
		delete[] cc_lvals;
...
template <typename Func, typename T>
void ClenshawCurtis<Func,T>::set_cc_tolerance(const double tol_in)
{
	cc_tolerance = tol_in;
}

template <typename Func, typename T>
void ClenshawCurtis<Func,T>::set_cc_tolerance_outer(const double tol_in)
{
	cc_tolerance_outer = tol_in;
}

template <typename Func, typename T>
T ClenshawCurtis<Func,T>::AdaptiveQuadCC(Func func, T a, T b, bool &converged, bool outer)
{
	T result = 0, result_old;
	double tolerance = (outer) ? cc_tolerance_outer : cc_tolerance;
	int i, level = 0, istep, istart;
	T abavg = (a+b)/2, abdif = (b-a)/2;
	...
	do {
		...
		result = 0;
		if (level==0) {
			funcptr[0] = func(abavg + abdif*cc_points[0]) + func(abavg - abdif*cc_points[0]);
			funcptr[cc_N-1] = func(abavg);
			result += cc_weights[0][1]*funcptr[cc_N-1];
		}
		for (j=1, i=istart; j < lval; j += 2, i += istep) {
			funcptr[i] = func(abavg + abdif*cc_points[i]) + func(abavg - abdif*cc_points[i]);
...
template <typename Func, typename T>
GaussPatterson<Func,T>::GaussPatterson()
{
	//pat_points = NULL;
	//pat_weights = NULL;
	//pat_orders = NULL;
...
template <typename Func, typename T>
void GaussPatterson<Func,T>::SetGaussPatterson(const double tol_in, const bool show_warnings)
{
...
template <typename Func, typename T>
void GaussPatterson<Func,T>::allocate_gauss_patterson_tables()
{
	if (pat_orders==NULL) {
...
template <typename Func, typename T>
void GaussPatterson<Func,T>::set_pat_tolerance_inner(const double tol_in)
{
	pat_tolerance = tol_in;
}

template <typename Func, typename T>
void GaussPatterson<Func,T>::set_pat_tolerance_outer(const double tol_in)
{
	pat_tolerance_outer = tol_in;
}

template <typename Func, typename T>
void GaussPatterson<Func,T>::deallocate_gauss_patterson_tables()
{
...
template <typename Func, typename T>
T GaussPatterson<Func,T>::AdaptiveQuad(Func func, T a, T b, bool &converged, bool outer)
{
