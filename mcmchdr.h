...
class UCMC : public Minimize, private LevenMarq, private Derivative
{
	...
	public:
		...
		void TWalk(const char *name, const double div, const int proj, const double din, const double alim, const double alimt, const double tol, const int Threads, double *best_fit_params, bool logfile, double** initial_points = NULL, string chain_info = "");
		void McmcAd(const char *, int);
		void Slicing(const char *, int, const char flag = NOTRANSFORM);
		void SlicingFull(const char *, int);
		void MonoSample(const char *name, const int N, double &lnZ, double *best_fit_params, double *parameter_errors, bool logfile, double** initial_points = NULL, string chain_info = "");
