...
void ImagePixelGrid::calculate_sourcepts_and_areas(const bool raytrace_pixel_centers, const bool verbal)
{
	...
	#pragma omp parallel
	{
		...
		#pragma omp for private(i,j,center_pt,center_srcpt) schedule(dynamic)
		for (j=0; j <= y_N; j++) {
			for (i=0; i <= x_N; i++) {
				if (((i < x_N) and (j < y_N)) and (lens->image_pixel_data->extended_mask[i][j])) {
					if (lens->split_imgpixels) {
						center_pt = subpixel_center_pts[i][j];
						center_srcpt = subpixel_center_sourcepts[i][j];
						for (subcell_index=0; subcell_index < SQR(nsplits[i][j]); subcell_index++) {
...
// Below is the MPI'd version, which does not ray trace the subpixel's. Maybe implement this someday and use the MPI'd version in case
// anyone wants to do likelihood evaluations over multiple nodes.
#ifdef USE_MPI
	MPI_Comm sub_comm;
	MPI_Comm_create(*(lens->group_comm), *(lens->mpi_group), &sub_comm);
#endif
...
// THIS NEEDS TO BE UPDATED, THE SAME WAY THE LMATRIX_SHAPELETS WAS: SUBPIXEL SOURCEPTS HAVE BEEN STORED DURING RAY TRACING AND SHOULD BE USED HERE
// DO THIS!!!!!!!!!!
		...
		if (split_imgpixels) {
			...
			for (img_index=0; img_index < image_npixels; img_index++) {
				...
				for (subcell_index=0; subcell_index < nsubpix; subcell_index++) {
					//if (at_least_one_lensed_shapelet) find_sourcept(center_pt[subcell_index],center_srcpt[subcell_index],thread,reference_zfactors,default_zsrc_beta_factors);
...
bool QLens::setup_convolution_FFT(const bool verbal)
{
	...
	//psf_transform = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*ncomplex);
	...
	//cout << "PSF_ZVEC: " << psf_zvec[0] << " " << psf_zvec[1] << " " << psf_zvec[2] << " " << psf_zvec[3] << endl;
	...
	//cout << "PSF_RVEC: " << psf_rvec[0] << " " << psf_rvec[1] << " " << psf_rvec[2] << " " << psf_rvec[3] << endl;
	//cout << "PSF_ZVEC: " << psf_zvec[0] << " " << psf_zvec[1] << " " << psf_zvec[2] << " " << psf_zvec[3] << endl;
	//cout << "PSF_TRAN: " << real(psf_transform[0]) << " " << imag(psf_transform[0]) << " " << real(psf_transform[1]) << " " << imag(psf_transform[1]) << endl;
...
void QLens::PSF_convolution_pixel_vector(double *surface_brightness_vector, const bool foreground, const bool verbal)
{
	...
	*/
...
#ifdef USE_FFTW
		int ncomplex = fft_nj*(fft_ni/2+1);
		//fftw_complex* img_transform = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*ncomplex);
#else
		int nzvec = 2*fft_ni*fft_nj;
	...
	//cout << "IMG_ZVEC: " << img_zvec[0] << " " << img_zvec[1] << " " << img_zvec[2] << " " << img_zvec[3] << endl;
	//cout << "IMG_TRAN: " << real(img_transform[0]) << " " << imag(img_transform[0]) << " " << real(img_transform[1]) << " " << imag(img_transform[1]) << endl;
