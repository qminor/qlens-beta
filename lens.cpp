	...
	if (n_pixellated_src > 0) {
		for (int i=0; i < n_pixellated_src; i++) {
			...
			if (source_fit_mode==Delaunay_Source) {
				...
				cout << "): grid undefined (set source mode to 'delaunay' or 'cartesian')";
			}
		}
	}
...
void QLens::remove_point_source(int ptsrc_number)
{
	...
	if (n_ptsrc > 1) {
		...
		new_image_data = new ImageData[n_ptsrc-1];
	}
...
void QLens::set_gridsize(double xl, double yl)
{
	...
	if ((n_extended_src_redshifts > 0) and (!image_pixel_data) and (data_pixel_size <= 0)) {
		// if an image pixel grid has been created, but it is not tied to a data image, then update the dimensions of this grid
		image_pixel_grids[0]->update_grid_dimensions(grid_xmin,grid_xmax,grid_ymin,grid_ymax);
	}
}
...
void QLens::set_grid_corners(double xmin, double xmax, double ymin, double ymax)
{
	...
	if ((n_extended_src_redshifts > 0) and (!image_pixel_data) and (data_pixel_size <= 0)) {
		// if an image pixel grid has been created, but it is not tied to a data image, then update the dimensions of this grid
		image_pixel_grids[0]->update_grid_dimensions(xmin,xmax,ymin,ymax);
	}
}
...
void QLens::set_img_npixels(const int npix_x, const int npix_y)
{
	...
	if ((n_extended_src_redshifts > 0) and (!image_pixel_data)) {
...
bool QLens::add_simulated_image_data(const lensvector &sourcept, const double srcflux)
{
	...
	image_data[n_ptsrc-1].input(n_images,imgs,err_pos,err_flux,sim_err_td,include_image,include_time_delays);
...
void QLens::write_point_image_data(string filename)
{
	...
	for (int i=0; i < n_ptsrc; i++) {
		outfile << image_data[i].n_images << " " << ptsrc_redshifts[ptsrc_redshift_idx[i]] << " # number of images, source redshift" << endl;
...
void QLens::sort_image_data_into_redshift_groups()
{
	...
	ImageData *sorted_image_data = new ImageData[n_ptsrc];
...
bool QLens::plot_srcpts_from_image_data(int dataset_number, ofstream* srcfile, const double srcpt_x, const double srcpt_y, const double flux)
{
	...
	for (i=0; i < n_srcpts; i++) {
		find_sourcept(image_data[dataset_number].pos[i],srcpts[i],0,specific_zfacs,specific_betafacs);
	}
	...
	if (include_time_delays) {
		...
		min_td_mod=1e30;
		for (i=0; i < n_srcpts; i++) {
			pot = potential(image_data[dataset_number].pos[i],specific_zfacs,specific_betafacs);
	...
	if (mpi_id==0) {
		...
		for (i=0; i < n_srcpts; i++) {
			...
			if (flux != -1) {
				imgflux = flux/inverse_magnification(image_data[dataset_number].pos[i],0,specific_zfacs,specific_betafacs);
				cout << "\t" << imgflux;
			}
			if (include_time_delays) {
...
vector<ImageDataSet> QLens::export_to_ImageDataSet()
{
	...
	image_data_sets.resize(n_ptsrc);
	int i,j;
	for (i=0; i < n_ptsrc; i++) {
		image_data_sets[i].set_n_images(image_data[i].n_images);
		image_data_sets[i].zsrc = ptsrc_redshifts[ptsrc_redshift_idx[i]];
...
bool QLens::initialize_fitmodel(const bool running_fit_in)
{
	if (source_fit_mode == Point_Source) {
		if (((!include_weak_lensing_chisq) or (weak_lensing_data.n_sources==0)) and ((n_ptsrc==0) or (image_data==NULL))) {
		...
		if (source_fit_mode==Shapelet_Source) {
			...
			if ((!found_shapelet) and (n_ptsrc==0)) { warn("no shapelet source object or source points found"); return false; }
		}
	}
		...
		for (j=0; j < image_data[i].n_images; j++) {
			if (image_data[i].use_in_chisq[j]) {
				if (use_magnification_in_chisq) {
					sourcept_jacobian(image_data[i].pos[j],beta_ji,jac,0,specific_zfacs,specific_betafacs);
					mag = jac.inverse();
					...
					find_sourcept(image_data[i].pos[j],beta_ji,0,specific_zfacs,specific_betafacs);
					siginv = 1.0/(SQR(image_data[i].sigma_pos[j]) + syserr_pos*syserr_pos);
...
double QLens::chisq_pos_source_plane()
{
	...
	for (i=0; i < n_ptsrc; i++) {
		...
		for (j=0; j < image_data[i].n_images; j++) {
			if (image_data[i].use_in_chisq[j]) {
				if (use_magnification_in_chisq) {
					sourcept_jacobian(image_data[i].pos[j],beta_ji[j],jac,0,ptsrc_zfactors[redshift_idx],ptsrc_beta_factors[redshift_idx]);
					mag = jac.inverse();
					mag00[j] = mag[0][0];
					mag01[j] = mag[0][1];
					mag11[j] = mag[1][1];
					...
					find_sourcept(image_data[i].pos[j],beta_ji[j],0,ptsrc_zfactors[redshift_idx],ptsrc_beta_factors[redshift_idx]);
					if (use_analytic_bestfit_src) {
						siginv = 1.0/(SQR(image_data[i].sigma_pos[j]) + syserr_pos*syserr_pos);
...
void QLens::find_analytic_srcflux(double *bestfit_flux)
{
	...
	for (i=0; i < n_ptsrc; i++) {
		...
		for (j=0; j < image_data[i].n_images; j++) {
			...
			hessian(image_data[i].pos[j],jac,ptsrc_zfactors[ptsrc_redshift_idx[i]],ptsrc_beta_factors[ptsrc_redshift_idx[i]]);
			jac[0][0] = 1 - jac[0][0];
			jac[1][1] = 1 - jac[1][1];
			jac[0][1] = -jac[0][1];
			jac[1][0] = -jac[1][0];
...
double QLens::chisq_time_delays()
{
	...
	for (k=0, i=0; i < n_ptsrc; i++) {
		...
		for (j=0; j < image_data[i].n_images; j++) {
			...
			find_sourcept(image_data[i].pos[j],beta_ij,0,specific_zfacs,specific_betafacs);
			pot = potential(image_data[i].pos[j],specific_zfacs,specific_betafacs);
...
double QLens::chisq_time_delays_from_model_imgs()
{
	...
	for (i=0; i < n_ptsrc; i++) {
		...
		for (k=0; k < image_data[i].n_images; k++) {
			for (j=0; j < n_images; j++) {
				distsqrs[n] = SQR(image_data[i].pos[k][0] - ptsrc_list[i]->images[j].pos[0]) + SQR(image_data[i].pos[k][1] - ptsrc_list[i]->images[j].pos[1]);
				data_k[n] = k;
				model_j[n] = j;
				n++;
			}
		...
		if (!skip) {
			for (k=0; k < image_data[i].n_images; k++) {
				...
				if (j != -1) {
					if (image_data[i].use_in_chisq[k]) {
						chisq_each_srcpt += SQR(ptsrc_list[i]->images[j].td - td_offset - image_data[i].time_delays[k])/sigsq;
					}
				} else {
...
double QLens::get_avg_ptsrc_dist(const int ptsrc_i)
{
	...
	if (image_data[ptsrc_i].n_images > 1) {
		...
		for (j=0; j < n_srcpts; j++) {
			find_sourcept(image_data[ptsrc_i].pos[j],srcpts[j],0,ptsrc_zfactors[ptsrc_redshift_idx[ptsrc_i]],ptsrc_beta_factors[ptsrc_redshift_idx[ptsrc_i]]);
			for (k=0; k < j; k++) {
...
bool QLens::setup_fit_parameters(const bool ignore_limits)
{
	...
	int expected_index = lensmodel_fit_parameters + srcmodel_fit_parameters + pixsrc_fit_parameters + pixlens_fit_parameters + ptsrc_fit_parameters;
...
bool QLens::get_ptsrc_parameter_numbers(const int pixsrc_i, int& pi, int& pf)
{
	...
	pf = dummy.size();
	pi += lensmodel_fit_parameters + srcmodel_fit_parameters + pixsrc_fit_parameters + pixlens_fit_parameters + pixlens_fit_parameters; // since lens, sb, pixsrc and pixlens fit parameters come before the ptsrc params
	pf += lensmodel_fit_parameters + srcmodel_fit_parameters + pixsrc_fit_parameters + pixlens_fit_parameters + pixlens_fit_parameters; // since lens, sb, pixsrc and pixlens fit parameters come before the ptsrc params
	if (pf==pi) return false;
	return true;
}
...
bool QLens::get_cosmo_parameter_numbers(int& pi, int& pf)
{
	...
	pi = lensmodel_fit_parameters + srcmodel_fit_parameters + pixsrc_fit_parameters + pixlens_fit_parameters + ptsrc_fit_parameters; // since lens, sb, pixsrc, pixlens and ptsrc fit parameters come before the cosmology params
	pf = pi + dummy.size();
	if (pf==pi) return false;
	return true;
}
...
bool QLens::get_misc_parameter_numbers(int& pi, int& pf)
{
	...
	pi = lensmodel_fit_parameters + srcmodel_fit_parameters + pixsrc_fit_parameters + pixlens_fit_parameters + ptsrc_fit_parameters + cosmo_fit_parameters; // since lens, sb, pixsrc, pixlens, ptsrc and cosmo fit parameters come before the miscellaneous params
	pf = pi + dummy.size();
	if (pf==pi) return false;
	return true;
}
...
{
	if ((nlens==0) and (n_ptsrc==0) and ((n_sb==0) or (source_fit_mode != Parameterized_Source))) { if (mpi_id==0) warn(warnings,"No lens/source model has been specified"); return false; }
		...
		if ((display_chisq_status) and (mpi_id==0)) {
			...
			if (include_imgpos_chisq) {
				if (used_imgplane_chisq) {
					...
					for (int i=0; i < n_ptsrc; i++) tot_data_images += image_data[i].n_images;
...
bool QLens::create_sourcegrid_cartesian(const int zsrc_i, const bool verbal, const bool use_mask, const bool autogrid_from_analytic_source, const bool image_grid_already_exists, const bool make_auxiliary_srcgrid)
{
...
bool QLens::create_sourcegrid_from_imggrid_delaunay(const bool use_weighted_srcpixel_clustering, const int zsrc_i, const bool verbal)
{
	...
	if ((use_srcpixel_clustering) or (use_weighted_srcpixel_clustering)) {
		...
		delaunay_srcgrids[src_i]->create_srcpixel_grid(src_centroids_x,src_centroids_y,n_src_centroids,ivals_centroids,jvals_centroids,n_image_pixels_x,n_image_pixels_y,find_invmag,zsrc_i);
...
bool QLens::create_lensgrid_cartesian(const int zsrc_i, const int pixlens_i, const bool verbal, const bool use_mask)
{
	...
	if (image_pixel_grids[zsrc_i] == NULL) {
		...
		image_pixel_grids[zsrc_i]->get_grid_params(xmin,xmax,ymin,ymax,npx,npy);
	}
	lensgrids[pixlens_i]->set_image_pixel_grid(image_pixel_grids[zsrc_i]);
	image_pixel_grids[zsrc_i]->set_lensgrid(lensgrids[pixlens_i]);
...
void QLens::plot_source_pixel_grid(const int zsrc_i, const char filename[])
{
	...
	SourcePixelGrid *cartesian_srcgrid = image_pixel_grids[zsrc_i]->cartesian_srcgrid;
...
void QLens::find_source_centroid(const int zsrc_i, double& xc_approx, double& yc_approx, const bool verbal)
{
	...
	if ((image_pixel_grids==NULL) or (image_pixel_grids[zsrc_i]==NULL)) {
		...
		image_pixel_grids[zsrc_i]->find_approx_source_size(xc_approx,yc_approx,verbal);
	}
}
...
bool QLens::load_image_surface_brightness_grid(string image_pixel_filename_root, const int hdu_indx, const bool show_fits_header)
{
	...
	if (fits_format == true) {
		if (data_pixel_size <= 0) { // in this case no pixel scale has been specified, so we simply use the grid that has already been chosen
			...
			xmin = grid_xcenter-0.5*grid_xlength; xmax = grid_xcenter+0.5*grid_xlength;
			ymin = grid_ycenter-0.5*grid_ylength; ymax = grid_ycenter+0.5*grid_ylength;
			image_pixel_data->set_grid_params(xmin,xmax,ymin,ymax); // these functions are defined in the header pixelgrid.h
		...
		status = image_pixel_data->load_data_fits(true,image_pixel_filename_root,hdu_indx,show_fits_header);
...
bool QLens::plot_lensed_surface_brightness(string imagefile, bool output_fits, bool plot_residual, bool plot_foreground_only, bool omit_foreground, bool show_all_pixels, bool normalize_residuals, bool offload_to_data, bool show_extended_mask, bool show_foreground_mask, bool show_noise_thresh, bool exclude_ptimgs, bool only_ptimgs, int specific_zsrc_i, bool show_only_first_order_corrections, bool plot_log, bool plot_current_sb, bool verbose)
{
	...
	if (!plot_current_sb) {
		for (int zsrc_i=zsrc_i_0; zsrc_i < zsrc_i_f; zsrc_i++) {
			...
			for (k=0; k < n_sb; k++) {
				if ((sb_list[k]->is_lensed) and (sbprofile_redshift_idx[k]==zsrc_i)) {
			...
			if (source_fit_mode==Cartesian_Source) {
				...
				if (at_least_one_lensed_nonshapelet_src) {
					at_least_one_foreground_src_included = true; // non-shapelet analytic sources get included in the "foreground" surface brightness (kinda weird, I know)
...
double QLens::pixel_log_evidence_times_two(double &chisq0, const bool verbal, const int ranchisq_i)
{
	...
	bool include_foreground_sbmask = false;
	bool include_foreground_sb = false;
	bool at_least_one_foreground_src = false;
	bool at_least_one_lensed_src = false;
	bool at_least_one_lensed_nonshapelet_src = false;
	bool at_least_one_shapelet_src = false;
	for (int k=0; k < n_sb; k++) {
	...
	if ((!ignore_foreground_in_chisq) and (at_least_one_foreground_src)) { include_foreground_sbmask = true; include_foreground_sb = true; } 
	...
	if ((source_fit_mode==Shapelet_Source) and (!at_least_one_shapelet_src) and ((n_ptsrc==0) or ((!include_imgfluxes_in_inversion) and (!include_srcflux_in_inversion)))) {
		warn("cannot perform inversion because no shapelet objects and no point sources with invertible amplitudes are defined");
		chisq0=-1e30; return -1e30;
	}
	...
	if (source_fit_mode == Cartesian_Source) {
		for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
			...
			if (cartesian_srcgrids[src_i]==NULL) die("cartesian source grid should not be NULL"); // just for debugging purposes; remove this line later
			image_pixel_grids[zsrc_i]->set_cartesian_srcgrid(cartesian_srcgrids[src_i]);
			...
			if (auto_srcgrid_npixels) {
				...
				if (assign_pixel_mappings(zsrc_i,false,verbal_now)==false) {
					return 2e30;
				}
			...
			*/
			...
			if (assign_pixel_mappings(zsrc_i,false,verbal)==false) {
				return 2e30;
			}
			...
			if ((mpi_id==0) and (verbal)) {
				...
				if (auto_srcgrid_npixels) {
					double high_sn_pixfrac = ((double) n_amps*high_sn_srcgrid_overlap_area/total_srcgrid_overlap_area) / image_pixel_grids[zsrc_i]->n_high_sn_pixels;
			...
			if (inversion_method==DENSE) create_lensing_matrices_from_Lmatrix_dense(zsrc_i,false,verbal);
			else create_lensing_matrices_from_Lmatrix(zsrc_i,dense_Fmatrix,false,verbal);
#ifdef USE_OPENMP
			if (show_wtime) {
			...
			if ((!optimize_regparam)) {
				if (inversion_method==MUMPS) invert_lens_mapping_MUMPS(zsrc_i,Fmatrix_log_determinant,verbal);
				else if (inversion_method==UMFPACK) invert_lens_mapping_UMFPACK(zsrc_i,Fmatrix_log_determinant,verbal);
		...
		for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
			...
			if (src_i < 0) {
				...
				if ((regularization_method != None) and (source_npixels > 0)) {
					...
					if ((include_potential_perturbations) and ((*regparam_pot_ptr) != 0)) {
						regterms = calculate_regularization_prior_term(regparam_pot_ptr,true);
						loglike_reg += regterms;
						logev_times_two += regterms;
					}

					if (!use_covariance_matrix) logev_times_two += Fmatrix_log_determinant;
		...
		for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
			if ((mpi_id==0) and (verbal)) cout << "Assigning foreground pixel mappings... (MAYBE REMOVE THIS FROM CHISQ AND DO AHEAD OF TIME?)\n";
			assign_foreground_mappings(zsrc_i); // note, this is done even if there is no foreground "source" object; foreground sb vector just has all elements equal zero
			if ((at_least_one_foreground_src) or (at_least_one_lensed_nonshapelet_src)) {
				if (!ignore_foreground_in_chisq) {
	...
	for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
		...
		chisq0 += chisq0_zsrc;
		logev_times_two += chisq0_zsrc; // logev_times_two includes the prior terms
	...
	if (((source_fit_mode==Cartesian_Source) or (source_fit_mode==Delaunay_Source) or (source_fit_mode==Shapelet_Source)) and (source_npixels > 0))
	{
		if ((group_id==0) and (logfile.is_open())) {
			if (sb_outside_window) logfile << " -2*log(ev)=" << logev_times_two << " (no priors; SB produced outside window)" << endl;
...
bool QLens::generate_and_invert_lensing_matrix_delaunay(const int zsrc_i, const int src_i, const bool potential_perturbations, const bool save_sb_gradient, double& tot_wtime, double& tot_wtime0, const bool verbal)
{
	if ((mpi_id==0) and (verbal)) cout << "Assigning pixel mappings...\n";
	...
	//if (potential_perturbations) image_pixel_grids[zsrc_i]->calculate_subpixel_source_gradient();
	if ((mpi_id==0) and (verbal)) cout << "Initializing pixel matrices...\n";
	initialize_pixel_matrices(zsrc_i,potential_perturbations,verbal);
	...
	if ((regularization_method != None) and (image_pixel_grids[zsrc_i]->delaunay_srcgrid != NULL)) {
		if (create_regularization_matrix(zsrc_i,include_lum_weighting,get_lumreg_from_sbweights,false,verbal)==false) { clear_pixel_matrices(); return false; } // in this case, covariance matrix was not positive definite 
		if ((potential_perturbations) and (create_regularization_matrix(zsrc_i,include_lum_weighting,get_lumreg_from_sbweights,true,verbal)==false)) { clear_pixel_matrices(); return false; } // in this case, covariance matrix was not positive definite 
	}
	...
	if ((mpi_id==0) and (verbal)) cout << "Creating lensing matrices...\n" << flush;
	bool dense_Fmatrix = ((inversion_method==DENSE) or (inversion_method==DENSE_FMATRIX)) ? true : false;
	if (inversion_method==DENSE) create_lensing_matrices_from_Lmatrix_dense(zsrc_i,potential_perturbations,verbal);
	else create_lensing_matrices_from_Lmatrix(zsrc_i,dense_Fmatrix,potential_perturbations,verbal);
#ifdef USE_OPENMP
	if (show_wtime) {
		tot_wtime = omp_get_wtime() - tot_wtime0;
		if (mpi_id==0) cout << "Total wall time before F-matrix inversion: " << tot_wtime << endl;
	...
	if ((optimize_regparam) and (regularization_method != None) and (!potential_perturbations) and (image_pixel_grids[zsrc_i]->delaunay_srcgrid != NULL)) {
		bool pre_srcgrid = ((use_lum_weighted_srcpixel_clustering) and (!use_saved_sbweights)) ? true : false;
		if (optimize_regularization_parameter(zsrc_i,dense_Fmatrix,verbal,pre_srcgrid)==false) { clear_pixel_matrices(); clear_sparse_lensing_matrices(); return false; }
	}
	if ((use_lum_weighted_srcpixel_clustering) and (!use_saved_sbweights)) {
		...
		if ((mpi_id==0) and (verbal)) cout << "Inverting lens mapping...\n" << flush;
		if ((optimize_regparam) and (regularization_method != None) and (!potential_perturbations) and (image_pixel_grids[zsrc_i]->delaunay_srcgrid != NULL)) {
			if (optimize_regularization_parameter(zsrc_i,dense_Fmatrix,verbal)==false) { clear_pixel_matrices(); clear_sparse_lensing_matrices(); return false; }
		}
	}
	...
	if (save_sb_gradient) image_pixel_grids[zsrc_i]->calculate_subpixel_source_gradient();
