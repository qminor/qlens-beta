...
int Lens::add_new_lens_redshift(const double zl, bool& new_redshift)
{
	...
	if (new_redshift) {
		...
		new_zlens_group_lens_indx_col[zlens_group_size[znum]] = nlens;
...
void Lens::remove_lens(int lensnumber)
{
	...
	remove_old_lens_redshift(lens_redshift_idx[lensnumber], lensnumber); // removes the lens redshift from the list if no other lenses share that redshift
...
bool Lens::create_grid(bool verbal, double *zfacs, double **betafacs, const int redshift_index) // the last (optional) argument indicates which images are being fit to; used to optimize the subgridding
{
	...
	if (subgrid_around_satellites) {
		subgrid_around_satellite_galaxies(centers,zfacs,betafacs,redshift_index);
		delete[] centers;
	}
...
void Lens::find_effective_lens_centers(lensvector *centers, double *zfacs, double **betafacs)
{
	...
	for (i=0; i < nlens; i++) {
		...
		if (zlsub > zlprim) {
			if (find_lensed_position_of_background_perturber(i,centers[i])==false) die("wtf!");
		} else {
...
void Lens::subgrid_around_satellite_galaxies(lensvector *centers, double *zfacs, double **betafacs, const int redshift_index)
{
	if (grid==NULL) {
