...
template <>
lensvector<double> *LensCalc<double>::defs = NULL;
template <>
lensvector<double> **LensCalc<double>::defs_subtot = NULL;
template <>
lensvector<double> *LensCalc<double>::defs_i = NULL;
template <>
lensvector<double> *LensCalc<double>::xvals_i = NULL;
template <>
lensmatrix<double> *LensCalc<double>::jacs = NULL;
template <>
lensmatrix<double> *LensCalc<double>::hesses = NULL;
template <>
lensmatrix<double> **LensCalc<double>::hesses_subtot = NULL;
template <>
lensmatrix<double> *LensCalc<double>::hesses_i = NULL;
template <>
lensmatrix<double> *LensCalc<double>::Amats_i = NULL;
#ifdef USE_STAN
template <>
lensvector<stan::math::var> *LensCalc<stan::math::var>::defs = NULL;
template <>
lensvector<stan::math::var> **LensCalc<stan::math::var>::defs_subtot = NULL;
template <>
lensvector<stan::math::var> *LensCalc<stan::math::var>::defs_i = NULL;
template <>
lensvector<stan::math::var> *LensCalc<stan::math::var>::xvals_i = NULL;
template <>
lensmatrix<stan::math::var> *LensCalc<stan::math::var>::jacs = NULL;
template <>
lensmatrix<stan::math::var> *LensCalc<stan::math::var>::hesses = NULL;
template <>
lensmatrix<stan::math::var> **LensCalc<stan::math::var>::hesses_subtot = NULL;
template <>
lensmatrix<stan::math::var> *LensCalc<stan::math::var>::hesses_i = NULL;
template <>
lensmatrix<stan::math::var> *LensCalc<stan::math::var>::Amats_i = NULL;
#endif
int *QLens::indxs = NULL;

void QLens::allocate_multithreaded_variables(const int& threads, const bool reallocate)
{
	if (LensCalc<double>::xvals_i != NULL) {
	...
	LensCalc<double>::defs_i = new lensvector<double>[nthreads];
	LensCalc<double>::jacs = new lensmatrix<double>[nthreads];
	LensCalc<double>::hesses = new lensmatrix<double>[nthreads];
	LensCalc<double>::hesses_subtot = new lensmatrix<double>*[nthreads];
	LensCalc<double>::Amats_i = new lensmatrix<double>[nthreads];
	LensCalc<double>::hesses_i = new lensmatrix<double>[nthreads];
	for (int i=0; i < nthreads; i++) {
		LensCalc<double>::defs_subtot[i] = new lensvector<double>[nmax_lens_planes];
		LensCalc<double>::hesses_subtot[i] = new lensmatrix<double>[nmax_lens_planes];
	}
#ifdef USE_STAN
	LensCalc<stan::math::var>::xvals_i = new lensvector<stan::math::var>[nthreads];
	LensCalc<stan::math::var>::defs = new lensvector<stan::math::var>[nthreads];
	LensCalc<stan::math::var>::defs_subtot = new lensvector<stan::math::var>*[nthreads];
	LensCalc<stan::math::var>::defs_i = new lensvector<stan::math::var>[nthreads];
	LensCalc<stan::math::var>::jacs = new lensmatrix<stan::math::var>[nthreads];
	LensCalc<stan::math::var>::hesses = new lensmatrix<stan::math::var>[nthreads];
	LensCalc<stan::math::var>::hesses_subtot = new lensmatrix<stan::math::var>*[nthreads];
	LensCalc<stan::math::var>::Amats_i = new lensmatrix<stan::math::var>[nthreads];
	LensCalc<stan::math::var>::hesses_i = new lensmatrix<stan::math::var>[nthreads];
	for (int i=0; i < nthreads; i++) {
		LensCalc<stan::math::var>::defs_subtot[i] = new lensvector<stan::math::var>[nmax_lens_planes];
		LensCalc<stan::math::var>::hesses_subtot[i] = new lensmatrix<stan::math::var>[nmax_lens_planes];
	}
#endif
}
...
{
	if (LensCalc<double>::xvals_i != NULL) {
		delete[] LensCalc<double>::xvals_i;
		delete[] LensCalc<double>::defs;
		delete[] LensCalc<double>::defs_i;
		delete[] LensCalc<double>::jacs;
		delete[] LensCalc<double>::hesses;
		delete[] LensCalc<double>::hesses_i;
		delete[] LensCalc<double>::Amats_i;
		for (int i=0; i < nthreads; i++) {
			delete[] LensCalc<double>::defs_subtot[i];
			delete[] LensCalc<double>::hesses_subtot[i];
		}
		delete[] LensCalc<double>::defs_subtot;
		delete[] LensCalc<double>::hesses_subtot;

		LensCalc<double>::xvals_i = NULL;
		LensCalc<double>::defs = NULL;
		LensCalc<double>::defs_i = NULL;
		LensCalc<double>::jacs = NULL;
		LensCalc<double>::hesses = NULL;
		LensCalc<double>::hesses_i = NULL;
		LensCalc<double>::Amats_i = NULL;
		LensCalc<double>::defs_subtot = NULL;
		LensCalc<double>::hesses_subtot = NULL;
	}
#ifdef USE_STAN
	if (LensCalc<stan::math::var>::xvals_i != NULL) {
		delete[] LensCalc<stan::math::var>::xvals_i;
		delete[] LensCalc<stan::math::var>::defs;
		delete[] LensCalc<stan::math::var>::defs_i;
		delete[] LensCalc<stan::math::var>::jacs;
		delete[] LensCalc<stan::math::var>::hesses;
		delete[] LensCalc<stan::math::var>::hesses_i;
		delete[] LensCalc<stan::math::var>::Amats_i;
		for (int i=0; i < nthreads; i++) {
			delete[] LensCalc<stan::math::var>::defs_subtot[i];
			delete[] LensCalc<stan::math::var>::hesses_subtot[i];
		}
		delete[] LensCalc<stan::math::var>::defs_subtot;
		delete[] LensCalc<stan::math::var>::hesses_subtot;

		LensCalc<stan::math::var>::xvals_i = NULL;
		LensCalc<stan::math::var>::defs = NULL;
		LensCalc<stan::math::var>::defs_i = NULL;
		LensCalc<stan::math::var>::jacs = NULL;
		LensCalc<stan::math::var>::hesses = NULL;
		LensCalc<stan::math::var>::hesses_i = NULL;
		LensCalc<stan::math::var>::Amats_i = NULL;
		LensCalc<stan::math::var>::defs_subtot = NULL;
		LensCalc<stan::math::var>::hesses_subtot = NULL;
	}
#endif
}
...
QLens::QLens(Cosmology* cosmo_in) : UCMC(), Model()
{
...
#elif defined(USE_EIGEN_INV)
	sparse_solver = EIGEN_SPARSE;
#else
	sparse_solver = NO_SPARSE;
#endif
...
void QLens::add_lens(LensProfile *new_lens)
{
	...
	add_new_lens_entry(new_lens->get_redshift());
...
void QLens::update_lens_redshift_data()
{
	int i;
	for (i=0; i < nlens; i++) {
		if (lens_redshifts[lens_redshift_idx[i]] != lens_list[i]->get_redshift()) {
			double new_zlens = lens_list[i]->get_redshift();
...
void QLens::update_ptsrc_redshift_data()
{
	int i;
	for (i=0; i < n_ptsrc; i++) {
		if (ptsrc_redshifts[ptsrc_redshift_idx[i]] != ptsrc_list[i]->ptsrc_params.zsrc) {
			double new_zsrc = ptsrc_list[i]->ptsrc_params.zsrc;
...
{
	if (band_number > n_model_bands) die("cannot add band with index that is > number of bands");
	srcgrids = new Model*[n_pixellated_src+1];
...
void QLens::find_pixellated_source_moments(const int npix, double& qs, double& phi_s, double& sigavg, double& xavg, double& yavg)
{
	if ((source_fit_mode==Delaunay_Source) and (auto_sourcegrid)) {
		for (int imggrid_i=0; imggrid_i < n_image_pixel_grids; imggrid_i++) {
			image_pixel_grids[imggrid_i]->find_optimal_sourcegrid<double,PlainTypes>(sourcegrid_xmin,sourcegrid_xmax,sourcegrid_ymin,sourcegrid_ymax,sourcegrid_limit_xmin,sourcegrid_limit_xmax,sourcegrid_limit_ymin,sourcegrid_limit_ymax); // this will just be for plotting purposes
		}
	}
...
void QLens::update_active_parameters(Model* param_object, const int pi)
{
...
bool QLens::update_parameter_list(const bool check_current_params) // if check_current_params=true, tests whether fitparams values match the actual values in the various model objects (and dies if there is a discrepancy)
{
	int n_fitparams;
	get_n_fit_parameters(n_fitparams);
	if (n_fitparams != param_list->nparams) die("RUHROH, number of parameters didn't match (%i vs %i)",n_fitparams,param_list->nparams);
...
bool QLens::create_grid(bool verbal, double *zfacs, double **betafacs, const int redshift_index, const bool force_store_cc_points) // the redshift_index argument (optional) indicates which images are being fit to; used to optimize the subgridding
{
	...
	if ((verbal) and (mpi_id==0)) {
		...
		if (show_wtime) {
			...
			if (mpi_id==0) cout << "Wall time for creating grid: " << mytime.count() << endl;
		}
	}
...
void QLens::find_effective_lens_centers_and_einstein_radii(lensvector<double> *centers, double *einstein_radii, int& i_primary, double *zfacs, double **betafacs, bool verbal)
{
	...
	for (i=0; i < nlens; i++) {
		if (zfacs[lens_redshift_idx[i]] != 0.0) {
			...
			if (einstein_radii[i] > largest_einstein_radius) {
				...
				zlprim = lens_list[i]->get_redshift();
				i_primary = i;
			}
		}
	}
	if (largest_einstein_radius==0) {
		...
		zlprim = lens_list[0]->get_redshift();
		i_primary = 0;
	}

	for (i=0; i < nlens; i++) {
		if (zfacs[lens_redshift_idx[i]] != 0.0) {
			zlsub = lens_list[i]->get_redshift();
...
bool QLens::calculate_perturber_subgridding_scale(int lens_number, bool* perturber_list, int host_lens_number, bool verbose, lensvector<double>& center, double& rmax_numerical, double *zfacs, double **betafacs)
{
	...
	double zlsub, zlprim;
	zlsub = lens_list[perturber_lens_number]->get_redshift();
	zlprim = lens_list[0]->get_redshift();
...
void QLens::calculate_critical_curve_perturbation_radius(int lens_number, bool verbose, double &rmax, double& mass_enclosed)
{
	...
	if (((lens_list[lens_number]->get_lenstype()!=dpie_LENS) and (lens_list[lens_number]->get_lenstype()!=sple_LENS)) or (lens_list[lens_number]->get_redshift() != lens_list[0]->get_redshift()))
	{
...
{
	double zlsub;
	zlsub = lens_list[lens_number]->get_redshift();
...
bool QLens::calculate_critical_curve_perturbation_radius_numerical(int lens_number, bool verbal, double& rmax_numerical, double& avg_sigma_enclosed, double& mass_enclosed, double& rmax_perturber_z, double &avgkap_scaled_to_primary_lensplane, bool subtract_unperturbed)
{
	...
	double zlsub, zlprim;
	zlsub = lens_list[perturber_lens_number]->get_redshift();
	zlprim = lens_list[primary_lens_number]->get_redshift();
...
bool QLens::find_caustic_minmax(double& min, double& max, double& max_minor_axis, double& theta_rmin, double& theta_rmax, int cc_num)
{
	if (!sorted_critical_curves) sort_critical_curves();
	...
	for (int k=0; k < npts; k++) {
		...
		if (rsq < rsqmin) {
			rsqmin = rsq;
			theta_rmin = get_angle(critical_curve->caustic_pts[k][0]-x_avg,critical_curve->caustic_pts[k][1]-y_avg);
		}
	}
...
double QLens::einstein_radius_of_primary_lens(const double zfac, double &reav)
{
	...
	if (show_wtime) {
		...
		if (mpi_id==0) cout << "Wall time for calculating Einstein radius: " << wtime.count() << endl;
	}
	delete[] centered;
...
double QLens::einstein_radius_single_lens(const double src_redshift, const int lensnum)
{
	...
	zfac = cosmo->kappa_ratio(lens_list[lensnum]->get_redshift(),src_redshift,reference_source_redshift);
...
double QLens::cc_xi_parameter(int cc_num)
{
	...
	for (m=0; m < npts; m++) {
		...
		xifac_avg += dkappa_cc_tot/(1-kappa_cc_tot); // sum up the values
	}
	xifac_avg /= npts; // divide through by number of points part of critical curve to average out
	delete[] include_lens;
	return (2*r_ein*xifac_avg+2); // full equation 
...
bool QLens::add_ptimage_data_from_unlensed_sourcepts(const bool include_errors_from_fisher_matrix, const int param_i, const double scale_errors)
{
	...
	for (i=0; i < n_images; i++) {
		...
		//imgs[i].pos[1] = ptsrc_list[i]->ptsrc_params.pos[1];
...
bool QLens::initialize_fitmodel(const bool running_fit_in)
{
	...
	if (n_pixellated_src > 0) {
		fitmodel->srcgrids = new Model*[n_pixellated_src];
	...
	if ((source_fit_mode != Point_Source) and (!redo_lensing_calculations_before_inversion)) {
		for (int i=0; i < n_extended_src_redshifts; i++) fitmodel->image_pixel_grids[i]->redo_lensing_calculations<PlainTypes>(); 
	}
...
bool QLens::update_lens_centers_from_pixsrc_coords()
{
	...
	for (int i=0; i < nlens; i++) {
		if (lens_list[i]->transform_center_coords_to_pixsrc_frame) {
			lens_list[i]->update_center_from_pixsrc_coords<double>(this);
			updated = true;
		}
	}
	return updated;
...
template <typename QScalar>
double QLens::update_model(const QScalar* params)
{
	bool status = true;
	...
	for (i=0; i < n_pixellated_src; i++) {
		...
		//if (srcgrids[i] != NULL) srcgrids[i]->update_fit_parameters_doub(temp_doub_params,index);
	}
	for (i=0; i < n_pixellated_lens; i++) {
		//lensgrids[i]->update_fit_parameters(params,index);
		lensgrids[i]->update_fit_parameters_doub(temp_doub_params,index);
	}
	for (i=0; i < n_ptsrc; i++) {
	...
	for (i=0; i < n_psf; i++) {
		...
		psf_list[i]->update_fit_parameters_doub(temp_doub_params,index);
	}
	...
	*/
	...
	update_fit_parameters_doub(temp_doub_params,index); // this is for the parameters that are part of the QLens class
	...
	delete[] temp_doub_params;
	return log_penalty_prior;
}
template double QLens::update_model<double>(const double* params);
#ifdef USE_STAN
template double QLens::update_model<stan::math::var>(const stan::math::var* params);
#endif
...
QScalar QLens::chisq_pos_source_plane()
{
	...
	QScalar delta_beta0;
	QScalar delta_beta1;
	QScalar delta_theta0;
	QScalar delta_theta1;
	...
	QScalar* mag10 = new QScalar[n_images_hi];
	...
	for (i=0; i < n_ptsrc; i++) {
		...
		for (j=0; j < point_image_data[i].n_images; j++) {
			if (point_image_data[i].images[j].use_in_chisq) {
				if (use_magnification_in_chisq) {
					...
					QScalar det = jac.j[0][0]*jac.j[1][1] - jac.j[0][1]*jac.j[1][0];
					mag00[j] = jac.j[1][1]/det;
					mag01[j] = -jac.j[1][0]/det;
					mag10[j] = -jac.j[0][1]/det;
					mag11[j] = jac.j[0][0]/det;
					...
					//} else {
						//cout << "Det = " << det << endl;
					//cout << mag00[j] << " " << mag01[j] << " " << mag11[j] << endl;
					//}
		...
		if (use_analytic_bestfit_src) {
			...
			beta = &src_bf;
			ptsrc_list[i]->update_srcpos(src_bf);
			//beta_x = src_bf[0];
			//beta_y = src_bf[1];
		} else {
#ifdef USE_STAN
		...
		//delta_beta[0] = SQR((*beta)[0] - beta_ji[0][0]);
		//delta_beta[1] = SQR((*beta)[1] - beta_ji[0][1]);
		//delta_theta[0] = mag00[0] * delta_beta[0] + mag01[0] * delta_beta[1];
		//delta_theta[1] = mag01[0] * delta_beta[0] + mag11[0] * delta_beta[1];
		//chisq += delta_beta.sqrnorm()/sigsq;

		for (j=0; j < point_image_data[i].n_images; j++) {
			if (point_image_data[i].images[j].use_in_chisq) {
				delta_beta[0] = (*beta)[0] - beta_ji[j][0];
				delta_beta[1] = (*beta)[1] - beta_ji[j][1];
				delta_beta0 =  (*beta)[0] - beta_ji[j][0];
				delta_beta1 =  (*beta)[1] - beta_ji[j][1];
				...
				if (use_magnification_in_chisq) {
					//delta_theta0 = mag00[j] * delta_beta0 + mag01[j] * delta_beta1;
					//delta_theta1 = mag01[j] * delta_beta0 + mag11[j] * delta_beta1;
					delta_theta0 = mag00[j]*delta_beta0 + mag10[j]*delta_beta1;
					delta_theta1 = mag01[j]*delta_beta0 + mag11[j]*delta_beta1;
					//delta_theta.v[0] = mag00[j] * delta_beta.v[0] + mag01[j] * delta_beta.v[1];
					//delta_theta.v[1] = mag01[j] * delta_beta.v[0] + mag11[j] * delta_beta.v[1];
					//delta_theta[0] = delta_beta[0];
					//delta_theta[1] = delta_beta[1];
					//chisq += delta_theta.sqrnorm() / sigsq + signormfac;
					chisq += (SQR(delta_theta0) + SQR(delta_theta1))/sigsq;
				} else {
...
#ifdef USE_STAN
template stan::math::var QLens::chisq_pos_source_plane<stan::math::var>();
#endif

/*
template<typename QScalar>
QScalar QLens::chisq_pos_source_plane_test(const QScalar beta_x, const QScalar beta_y)
{
	int i,j;
	QScalar chisq=0;
	int n_images_hi=0;
	...
	for (i=0; i < n_ptsrc; i++) {
		...
		for (j=0; j < point_image_data[i].n_images; j++) {
			if (point_image_data[i].images[j].use_in_chisq) {
				delta_beta[0] = beta_x - beta_ji[j][0];
				delta_beta[1] = beta_y - beta_ji[j][1];
				delta_beta0 =  beta_x - beta_ji[j][0];
				delta_beta1 =  beta_y - beta_ji[j][1];
				sigsq = SQR(point_image_data[i].images[j].sigma_pos);
				if (syserr_pos != 0.0) {
					 signormfac = 2*log(1.0 + syserr_pos*syserr_pos/sigsq);
					 sigsq += syserr_pos*syserr_pos;
...
template double QLens::chisq_pos_source_plane_test<double>(const double beta_x, const double beta_y);
#ifdef USE_STAN
template stan::math::var QLens::chisq_pos_source_plane_test<stan::math::var>(const stan::math::var beta_x, const stan::math::var beta_y);
#endif
*/
...
#ifdef USE_STAN
template stan::math::var QLens::chisq_pos_image_plane<stan::math::var>();
#endif
...
QScalar QLens::chisq_pos_image_plane_diagnostic(const bool verbose, const bool output_residuals_to_file, double &rms_imgpos_err, int &n_matched_images, const string output_filename)
{
	...
	int n_matched_images_each_srcpt;
	QScalar sigsq, signormfac, chisq_each_srcpt, rms_err_each_srcpt, dist;
	rms_imgpos_err = 0;
	n_matched_images = 0;
...
#ifdef USE_STAN
template stan::math::var QLens::chisq_pos_image_plane_diagnostic<stan::math::var>(const bool verbose, const bool output_residuals_to_file, double &rms_imgpos_err, int &n_matched_images, const string output_filename);
#endif
...
void QLens::find_analytic_srcflux(double *bestfit_flux)
{
	...
	for (i=0; i < n_ptsrc; i++) {
		...
		for (j=0; j < point_image_data[i].n_images; j++) {
			...
			hessian_at_datapt<double>(point_image_data[i].images[j].pos,jac,ptsrc_zfactors[ptsrc_redshift_idx[i]],ptsrc_beta_factors[ptsrc_redshift_idx[i]]);
			jac[0][0] = 1 - jac[0][0];
			jac[1][1] = 1 - jac[1][1];
			jac[0][1] = -jac[0][1];
			jac[1][0] = -jac[1][0];
...
void QLens::set_analytic_srcflux(const bool verbal)
{
	...
	for (int i=0; i < n_ptsrc; i++) {
		//ptsrc_list[i]->set_srcflux(srcflux[i]);
#ifdef USE_STAN
			if constexpr (std::is_same_v<double, stan::math::var>) {
				ptsrc_list[i]->set_srcflux_autodif(srcflux[i]); // although we're not using it directly, set the source object's flux for consistency
			} else
#endif
			ptsrc_list[i]->set_srcflux(srcflux[i]); // although we're not using it directly, set the source object's flux for consistency
...
QScalar QLens::chisq_flux()
{
	...
	for (i=0; i < n_ptsrc; i++) {
		...
		for (j=0; j < point_image_data[i].n_images; j++) {
			if (point_image_data[i].images[j].sigma_flux==0) { k++; continue; }
			hessian_at_datapt<QScalar>(point_image_data[i].images[j].pos,jac,ptsrc_zfactors[ptsrc_redshift_idx[i]],ptsrc_beta_factors[ptsrc_redshift_idx[i]]);
			jac[0][0] = 1 - jac[0][0];
			jac[1][1] = 1 - jac[1][1];
			jac[0][1] = -jac[0][1];
			jac[1][0] = -jac[1][0];
...
#ifdef USE_STAN
			if constexpr (std::is_same_v<QScalar, stan::math::var>) {
				flux_src = ptsrc_list[i]->get_srcflux_autodif(); // only one source flux value is currently supported; later this should be generalized so that
			} else
#endif
			flux_src = ptsrc_list[i]->get_srcflux(); // only one source flux value is currently supported; later this should be generalized so that
...
#ifdef USE_STAN
			if constexpr (std::is_same_v<QScalar, stan::math::var>) {
				ptsrc_list[i]->set_srcflux_autodif(flux_src); // although we're not using it directly, set the source object's flux for consistency
			} else
#endif
			ptsrc_list[i]->set_srcflux(flux_src); // although we're not using it directly, set the source object's flux for consistency
		}
...
#ifdef USE_STAN
template stan::math::var QLens::chisq_flux<stan::math::var>();
#endif
...
#ifdef USE_STAN
template stan::math::var QLens::chisq_time_delays<stan::math::var>();
#endif
...
#ifdef USE_STAN
template stan::math::var QLens::chisq_time_delays_from_model_imgs<stan::math::var>();
#endif
...
#ifdef USE_STAN
template stan::math::var QLens::chisq_weak_lensing<stan::math::var>();
#endif
...
double QLens::chisq_single_evaluation(const bool init_fitmodel, const bool show_total_wtime, const bool show_wtime_temp, const bool show_diagnostics, const bool show_status, const bool show_lensinfo, const bool find_gradient, const bool test_gradient)
{
	...
	if (init_fitmodel) {
		...
		if (show_wtime) {
			wtime = std::chrono::steady_clock::now() - wtime0;
			if ((mpi_id==0) and (show_status)) cout << "Wall time for initializing fitmodel (not part of likelihood evaluation): " << wtime.count() << endl;
		}
	} else {
		fitmodel = this;
		if (show_wtime) {
		...
		if ((source_fit_mode != Point_Source) and ((image_pixel_grids == NULL) or (image_pixel_grids[0]==NULL))) {
			...
			if (success) {
				if (show_wtime) {
					wtime = std::chrono::steady_clock::now() - wtime0;
					if ((mpi_id==0) and (show_status)) cout << "Wall time for initializing image pixel grids (not part of likelihood evaluation): " << wtime.count() << endl;
				}
			}
		}
	}
	...
	std::chrono::steady_clock::time_point chisq_wtime0;
	std::chrono::duration<double> chisq_wtime;
	...
	if (find_gradient) {
		...
		auto get_grad_finite_diff = [this](const double* params, double* grad, const Vector<double>& steps, const int n, const double h) {
			...
			for (int k=0; k < n; k++) {
				//x[k] = params[k];
				//f = (this->*LogLikePtr)(x);
				//cout << "param " << k << ": x=" << x[k] << ", f 2*loglike = " << (2*value_of(f)) << endl;
				x[k] = params[k]+h*steps[k];
				fp = (this->*LogLikePtr)(x);
				///cout << "param " << k << ": xp=" << x[k] << ", fp 2*loglike = " << (2*value_of(fp)) << endl;
				x[k] = params[k] - h*steps[k];
				fm = (this->*LogLikePtr)(x);
		...
		auto get_grad_ridders = [this](const double* params, double* grad, const Vector<double>& steps, const int n) {
		...
		stan::math::start_nested();
		{
			...
			loglike_stan.grad();
			if ((show_wtime) or (show_total_wtime)) {
				chisq_wtime = std::chrono::steady_clock::now() - chisq_wtime0;
				if ((mpi_id==0) and (show_status)) cout << "Wall time for autodiff likelihood evaluation (+gradients): " << chisq_wtime.count() << endl;
			}
...
class LogLikeGrad_Func
{
	...
	void ridders_method(const Eigen::VectorXd& params, Eigen::VectorXd& grad) {
		const double CON=1.4, CON2=(CON*CON);
		const double BIG=1.0e100;
		const double SAFE=2.0;
		const int NTAB = 10;
...
*/
	...
	Eigen::VectorXd paramvec = Eigen::Map<Eigen::VectorXd>(fitparams,n_fitparams);
	LBFGSBSolver<double> solver(param);
	Eigen::VectorXd lb = Eigen::Map<Eigen::VectorXd>(param_list->prior_limits_lo,n_fitparams);
	Eigen::VectorXd ub = Eigen::Map<Eigen::VectorXd>(param_list->prior_limits_hi,n_fitparams);
...
void QLens::output_fit_results(double *fitparams, Vector<double> &stepsizes, const double chisq_bestfit, const int chisq_evals, const bool show_parameter_errors, const bool used_autodiff)
{
	...
	fitmodel->update_model<double>(transformed_params);
...
double QLens::loglike_deriv(const Vector<QScalar> &params, const int index, const double step)
{
...
#endif
	{
		...
		for (int i=0; i < param_list->nparams; i++) {
			params_doub[i] = value_of(params[i]);
		}
...
void QLens::nested_sampling()
{
	...
	if (show_wtime) {
		wt = std::chrono::steady_clock::now() - wt0;
		if (mpi_id==0) cout << "Time for nested sampling: " << wt.count() << endl;
	}
...
void QLens::multinest(const bool resume_previous, const bool skip_run)
{
	...
	int *pWrap = new int[n_fitparams];				// which parameters to have periodic boundary conditions?
...
void QLens::chi_square_twalk()
{
	...
	if (show_wtime) {
		wt = std::chrono::steady_clock::now() - wt0;
		if (mpi_id==0) cout << "Time for T-Walk: " << wt.count() << endl;
	}
	if (mpi_id==0) {
		output_bestfit_model();
	}
...
bool QLens::adopt_model(Vector<double> &fitpars)
{
	...
	double log_penalty_prior = update_model<double>(param_list->untransformed_values); // the model is adopted here
	...
	if ((n_extended_src_redshifts > 1) and (source_fit_mode != Point_Source)) {
		...
		if (transform_center_coords) {
			if ((image_pixel_grids == NULL) or (image_pixel_grids[0]==NULL)) {
				...
				for (i=0; i < n_extended_src_redshifts; i++) {
					image_pixel_grids[i]->redo_lensing_calculations<PlainTypes>(false);
				}
			}
...
#ifdef USE_OPENMP
			wtime = std::chrono::steady_clock::now() - wtime0;
			if (mpi_id==0) cout << "Wall time for calculating 2D histograms: " << wtime.count() << endl;
#endif
		}
...
bool QLens::output_coolest_files(const string filename)
{
	...
	for (i=nlens-1; i >= 0; i--) { // adding lenses in reverse because the other lens modelers put the shear model before PEMD, so just to make it look the same
		...
		lens["redshift"] = lensptr->get_redshift();
		lens["mass_model"] = Json::Value(Json::arrayValue);
		lens["light_model"] = Json::Value(Json::arrayValue);
...
QScalar QLens::fitmodel_loglike_point_source(const QScalar* params)
{
	...
	QScalar loglike=0, chisq_total=0, chisq;
	QScalar log_penalty_prior;
	int n_fitparams = param_list->nparams;
	QScalar* transformed_params = new QScalar[n_fitparams];
	if (params != NULL) {
			...
			if (include_imgpos_chisq) {
				if (used_imgplane_chisq) {
					...
					if (fitmodel->chisq_it % chisq_display_frequency == 0) {
						...
						if (syserr_pos != 0.0) {
							QScalar signormfac, chisq_sys = chisq;
							int i,k;
							for (i=0; i < n_ptsrc; i++) {
								for (k=0; k < point_image_data[i].n_images; k++) {
	...
	if (include_flux_chisq) {
		chisq = fitmodel->chisq_flux<QScalar>();
		chisq_total += chisq;
		if ((display_chisq_status) and (mpi_id==0)) {
	...
	if (include_time_delay_chisq) {
		chisq = fitmodel->chisq_time_delays<QScalar>();
		chisq_total += chisq;
		if ((display_chisq_status) and (mpi_id==0)) {
			if (showed_first_chisq) cout << ", ";
			else showed_first_chisq = true;
	...
	if (include_weak_lensing_chisq) {
		chisq = fitmodel->chisq_weak_lensing<QScalar>();
		chisq_total += chisq;
		if ((display_chisq_status) and (mpi_id==0)) {
			if (showed_first_chisq) cout << ", ";
			else showed_first_chisq = true;
...
template double QLens::fitmodel_loglike_point_source<double>(const double* params);
#ifdef USE_STAN
...
QScalar QLens::fitmodel_loglike_extended_source(const QScalar* params)
{
	...
	std::chrono::duration<double> update_wtime;
	if (show_wtime) {
		update_wtime0 = std::chrono::steady_clock::now();
	}
	int n_fitparams = param_list->nparams;
	QScalar* transformed_params = new QScalar[n_fitparams];
	QScalar loglike=0, chisq=0, chisq0, chisq_td;
	QScalar log_penalty_prior;
	if (params != NULL) {
		fitmodel->param_list->inverse_transform_parameters(params,transformed_params);
	...
	if (show_wtime) {
		update_wtime = std::chrono::steady_clock::now() - update_wtime0;
		if (mpi_id==0) cout << "wall time for updating parameters: " << update_wtime.count() << endl;
	}
...
//#endif
		...
		double chisq00;
		double chisq_doub=0;
		double chisq0_doub=0;
		if (loglike < 1e30) {
			for (int i=0; i < n_ranchisq; i++) {
				chisq_doub += fitmodel->pixel_log_evidence_times_two(chisq00,false,i);
				chisq0_doub += chisq00;
			}
			chisq_doub /= n_ranchisq;
			chisq0_doub /= n_ranchisq;
		}
		chisq = chisq_doub;
		chisq0 = chisq0_doub;
	}
...
double QLens::mass2d_r(const double r, const int lensnum, const bool use_kpc)
{
	...
	z = lens_list[lensnum]->get_redshift();
...
{
	double z = lens_list[lensnum]->get_redshift();
...
bool QLens::create_sourcegrid_cartesian(const int band_number, const int zsrc_i, const bool verbal, const bool use_mask, const bool autogrid_from_analytic_source, const bool image_grid_already_exists, const bool make_auxiliary_srcgrid)
{
	...
	QScalar srcgrid_xmin_auto, srcgrid_xmax_auto, srcgrid_ymin_auto, srcgrid_ymax_auto;
	if (use_image_pixelgrid) {
		...
		if (!image_grid_already_exists) {
			...
			if (image_pixel_grids[imggrid_i] == NULL) {
				//image_pixel_grid = new ImagePixelGrid(this,source_fit_mode,xmin,xmax,ymin,ymax,n_image_pixels_x,n_image_pixels_y,0);
				bool raytrace = true;
		...
		if (auto_sourcegrid) {
			if (source_fit_mode != Delaunay_Source) {
				if ((autogrid_from_analytic_source) and (at_least_one_lensed_src)) {
					...
					image_pixel_grids[imggrid_i]->find_optimal_sourcegrid<QScalar,MathTypes>(srcgrid_xmin_auto,srcgrid_xmax_auto,srcgrid_ymin_auto,srcgrid_ymax_auto,sourcegrid_limit_xmin,sourcegrid_limit_xmax,sourcegrid_limit_ymin,sourcegrid_limit_ymax);
				}
			} else {
				...
				image_pixel_grids[imggrid_i]->set_sourcegrid_params_from_ray_tracing<MathTypes>(srcgrid_xmin_auto,srcgrid_xmax_auto,srcgrid_ymin_auto,srcgrid_ymax_auto,sourcegrid_limit_xmin,sourcegrid_limit_xmax,sourcegrid_limit_ymin,sourcegrid_limit_ymax);
			}
		}
		...
		if ((auto_srcgrid_npixels) and (!make_auxiliary_srcgrid)) {
			if (auto_srcgrid_set_pixel_size) // this option doesn't work well, DON'T USE RIGHT NOW
				image_pixel_grids[imggrid_i]->find_optimal_firstlevel_sourcegrid_npixels(sourcegrid_xmin,sourcegrid_xmax,sourcegrid_ymin,sourcegrid_ymax,srcgrid_npixels_x,srcgrid_npixels_y,img_npixels);
			else
				image_pixel_grids[imggrid_i]->find_optimal_sourcegrid_npixels(sourcegrid_xmin,sourcegrid_xmax,sourcegrid_ymin,sourcegrid_ymax,srcgrid_npixels_x,srcgrid_npixels_y,img_npixels);
			if ((verbal) and (mpi_id==0)) {
				...
				cout << "Number of active image pixels expected: " << img_npixels << endl;
			}
		}
		...
		srcgrid_xmin_auto = sourcegrid_xmin;
		srcgrid_xmax_auto = sourcegrid_xmax;
		srcgrid_ymin_auto = sourcegrid_ymin;
		srcgrid_ymax_auto = sourcegrid_ymax;
	}

	if (auto_sourcegrid) {
		if (cartesian_srcgrids[src_i]->cartesian_srcgrid_params.srcgrid_size_scale != 0) {
			QScalar xwidth_adj = cartesian_srcgrids[src_i]->cartesian_srcgrid_params.srcgrid_size_scale*(srcgrid_xmax_auto-srcgrid_xmin_auto);
			QScalar ywidth_adj = cartesian_srcgrids[src_i]->cartesian_srcgrid_params.srcgrid_size_scale*(srcgrid_ymax_auto-srcgrid_ymin_auto);
			QScalar srcgrid_xc, srcgrid_yc;
			srcgrid_xc = (srcgrid_xmax_auto + srcgrid_xmin_auto)/2;
			srcgrid_yc = (srcgrid_ymax_auto + srcgrid_ymin_auto)/2;
			srcgrid_xmin_auto = srcgrid_xc - xwidth_adj/2;
			srcgrid_xmax_auto = srcgrid_xc + xwidth_adj/2;
			srcgrid_ymin_auto = srcgrid_yc - ywidth_adj/2;
			srcgrid_ymax_auto = srcgrid_yc + ywidth_adj/2;
		}
	}
	...
	cartesian_srcgrids[src_i]->create_pixel_grid(this,srcgrid_xmin_auto,srcgrid_xmax_auto,srcgrid_ymin_auto,srcgrid_ymax_auto,nsplitx,nsplity);
	if ((mpi_id==0) and (verbal)) {
...
bool QLens::create_sourcegrid_delaunay(const int src_i, const bool use_mask, const bool verbal)
{
	...
	create_sourcegrid_from_imggrid_delaunay<PlainTypes>(false,band_number,zsrc_i,verbal);
	delaunay_srcgrids[src_i]->assign_surface_brightness_from_analytic_source<double>(zsrc_i);
...
bool QLens::create_sourcegrid_from_imggrid_delaunay(const bool use_weighted_srcpixel_clustering, const int band_number, const int zsrc_i, const bool verbal)
{
	...
	int imggrid_i = band_number*n_extended_src_redshifts + zsrc_i;
	ImgGrid_Params<MathTypes>& imggrid_params = image_pixel_grids[imggrid_i]->assign_imggrid_param_object<MathTypes>();
	...
	DelaunaySourceGrid_Params<QScalar>& delaunay_srcgrid_params = delaunay_srcgrids[src_i]->assign_delaunay_srcgrid_param_object<QScalar>();

	ImageData *image_data;
	if (n_data_bands > band_number) image_data = imgdata_list[band_number]; // temporary until I sort out the band stuff
	else image_data = NULL;

	QScalar *srcpts_x, *srcpts_y;
	int *ivals, *jvals;
	int *pixptr_i, *pixptr_j;
	int npix_in_mask;
	if (imggrid_i >= n_image_pixel_grids) die("image grid index does not exist");
	//if (include_fgmask_in_inversion) {
		//npix_in_mask = image_pixel_grids[zsrc_i]->image_npixels_emask;
		//pixptr_i = image_pixel_grids[zsrc_i]->emask_pixels_i;
		//pixptr_j = image_pixel_grids[zsrc_i]->emask_pixels_j;
	//} else {
		npix_in_mask = image_pixel_grids[imggrid_i]->image_npixels;
		pixptr_i = image_pixel_grids[imggrid_i]->mask_pixels_i;
		pixptr_j = image_pixel_grids[imggrid_i]->mask_pixels_j;
	//}
	double avg_sb = -1e30;
	if (image_data) {
	...
	int nsubpix_per_pixel, nysubpix;
	double sb;
	...
	nysubpix = image_pixel_grids[imggrid_i]->imgpixel_nsplit;
	nsubpix_per_pixel = image_pixel_grids[imggrid_i]->n_subpix_per_pixel;
	for (n=0; n < npix_in_mask; n++) {
		...
		if ((include_fgmask_in_inversion) and (!(image_data->in_mask[zsrc_i][i][j]))) {
			...
			include = false;
			if ((do_clustering) or (delaunay_mode==5)) {
				include = true;
	...
	srcpts_x = new QScalar[npix];
	srcpts_y = new QScalar[npix];
	...
	int nsplit, n_subpixel=0;
	nsplit = image_pixel_grids[imggrid_i]->imgpixel_nsplit;
	nsubpix_per_pixel = image_pixel_grids[imggrid_i]->n_subpix_per_pixel;
	int ii,jj;
	for (n=0; n < npix_in_mask; n++) {
		i = pixptr_i[n];
		...
		if (include_in_delaunay_grid[n]) {
			if ((!split_imgpixels) or ((delaunay_from_pixel_centers) and (!do_clustering))) {
				...
				srcpts_x[npix] = imggrid_params.srcpt_x_centers(n);
				srcpts_y[npix] = imggrid_params.srcpt_y_centers(n);
			} else {
				if ((do_clustering) or (delaunay_mode==5)) {
					for (int k=0; k < nsubpix_per_pixel; k++) {
						ii = i*nsplit + (nsubpix_per_pixel-1-k) / nsplit; // I don't remember why we do the subpixels in reverse order here...
						jj = j*nsplit + (nsubpix_per_pixel-1-k) % nsplit;
						n_subpixel = image_pixel_grids[imggrid_i]->subpixel_index_ss[ii][jj];
						srcpts_x[npix] = imggrid_params.srcpt_x_subpixel_centers(n_subpixel);
						srcpts_y[npix] = imggrid_params.srcpt_y_subpixel_centers(n_subpixel);
						//srcpts_x[npix] = image_pixel_grids[imggrid_i]->subpixel_center_sourcepts[i][j][nsubpix_per_pixel-1-k][0];
						//srcpts_y[npix] = image_pixel_grids[imggrid_i]->subpixel_center_sourcepts[i][j][nsubpix_per_pixel-1-k][1];
						ivals[npix] = i;
						jvals[npix] = j;
						if (use_weighted_srcpixel_clustering) wfactors[npix] = image_pixel_grids[imggrid_i]->subpixel_weights[i][j][nsubpix_per_pixel-1-k];
						npix++;
					}
				} else {
					if (use_random_delaunay_srcgrid) {
						subcell_i1 = (int) (nsubpix_per_pixel*RandomNumber());
						//subcell_i2 = (int) (nsubpix_per_pixel*RandomNumber());
					} else {
						subcell_i1 = nsubpix_per_pixel-1 - ((i+2*j) % nsubpix_per_pixel); // this is really only optimized for 2x2 splittings
						//subcell_i2 = nsubpix_per_pixel-1 - ((i+2*j+2) % nsubpix_per_pixel); // this is really only optimized for 2x2 splittings
					}
					ii = i*nsplit + subcell_i1 / nsplit;
					jj = j*nsplit + subcell_i1 % nsplit;
					n_subpixel = image_pixel_grids[imggrid_i]->subpixel_index_ss[ii][jj];
					srcpts_x[npix] = imggrid_params.srcpt_x_subpixel_centers(n_subpixel);
					srcpts_y[npix] = imggrid_params.srcpt_y_subpixel_centers(n_subpixel);
				}
			...
			if ((!do_clustering) and (delaunay_mode != 5)) {
				ivals[npix] = i;
				jvals[npix] = j;
				npix++;
			}
	...
	if (do_clustering) {
#ifdef USE_MLPACK
		int *iweights_norm;
		QScalar min_weight = 1e30;
		QScalar *input_data = new QScalar[2*npix];
		QScalar *weights = new QScalar[npix];
		QScalar *initial_centroids;
		int *ivals_centroids;
		int *jvals_centroids;
		...
		if (!use_weighted_srcpixel_clustering) {
			...
			for (i=0; i < npix; i++) {
				...
				weights[i] = pow(wfactors[i]+delaunay_srcgrid_params.alpha_clus,delaunay_srcgrid_params.beta_clus);
		...
		initial_centroids = new QScalar[2*n_src_centroids];
		...
		if constexpr (stan::is_autodiff_v<QScalar>) {
			...
			delete[] input_data_doub;
			delete[] initial_centroids_doub;
			delete[] weights_doub;
		} else
#endif
		{
			dataset = arma::mat(input_data, 2, npix);
			weightvec = arma::Col<double> (weights, npix);
			centroids = arma::mat(initial_centroids, 2, n_src_centroids);
		}
...
#endif
				{
					dataset2 = arma::mat(input_data, 2, npix);
					weightvec2 = arma::Col<double> (weights, npix);
					centroids2 = arma::mat(initial_centroids, 2, n_src_centroids);
				}
...
bool QLens::create_lensgrid_from_imggrid_delaunay(const int zsrc_i, const int pixlens_i, const bool verbal)
{
	...
	if (include_fgmask_in_inversion) {
		...
		pixptr_i = image_pixel_grids[zsrc_i]->mask_pixels_i;
		
		pixptr_j = image_pixel_grids[zsrc_i]->mask_pixels_j;
	}
	double avg_sb = -1e30;
	...
	for (n=0; n < npix_in_mask; n++) {
		...
		i = pixptr_i[n];
		j = pixptr_j[n];
		nysubpix = image_pixel_grids[zsrc_i]->imgpixel_nsplit; // why not just store the square and avoid having to always take the square?
	...
	for (n=0; n < npix_in_mask; n++) {
		...
		if (include_in_delaunay_grid[n]) {
			if ((!split_imgpixels) or ((delaunay_from_pixel_centers) and (!use_lenspixel_clustering))) {
				...
				nsubpix = SQR(image_pixel_grids[zsrc_i]->imgpixel_nsplit; // why not just store the square and avoid having to always take the square?
				if ((use_lenspixel_clustering) or (delaunay_mode==5)) {
...
const bool QLens::output_lensed_surface_brightness(Vector<double>& xvals, Vector<double>& yvals, Vector<double>& zvals, const int band_number, const bool output_fits, const bool plot_residual, bool plot_foreground_only, const bool omit_foreground, const bool show_all_pixels, const bool normalize_residuals, const bool offload_to_data, const bool show_extended_mask, const bool show_foreground_mask, const bool show_noise_thresh, const bool exclude_ptimgs, const bool only_ptimgs, int specific_zsrc_i, const bool show_only_first_order_corrections, const bool plot_log, const bool plot_current_sb, const bool verbose)
{
	...
	if (!plot_current_sb) {
		for (zsrc_i=zsrc_i_0, imggrid_i=primary_imggrid_i; zsrc_i < zsrc_i_f; zsrc_i++, imggrid_i++) {
			if (image_pixel_grids[imggrid_i] == NULL) {
				...
				if (use_data) {
					image_pixel_grids[imggrid_i] = new ImagePixelGrid(this, source_fit_mode, (*image_data), include_fgmask_in_inversion, band_number, zsrc_i, imggrid_i, assigned_mask[imggrid_i]);
					if (band_number < n_psf) psf_list[band_number]->set_image_pixel_grid(image_pixel_grids[imggrid_i]);

				}
				else {
					image_pixel_grids[imggrid_i] = new ImagePixelGrid(this,source_fit_mode,xmin,xmax,ymin,ymax,n_image_pixels_x,n_image_pixels_y,false,band_number,zsrc_i,imggrid_i);
			...
			bool raytrace_pixel_centers = false;
			if (changed_mask) {
				image_pixel_grid->setup_ray_tracing_arrays();
				raytrace_pixel_centers = true; // is this necessary? Note, this setting only matters if pixel splitting is used, because if not, it will ray trace the centers anyway (needed for loglikelihood calculation)
			}
			image_pixel_grid->calculate_sourcepts_and_areas<PlainTypes>(raytrace_pixel_centers); // unlike the above line, we don't reinitialize the arrays since mask hasn't changed
			...
			if (at_least_one_inverted_or_lensed_src_object) {
				...
				if (source_fit_mode==Parameterized_Source) {
					image_pixel_grid->find_surface_brightness_vec<PlainTypes>(show_extended_mask,fg_only,lensed_only,include_noninverted_src_as_foreground); // the last argument will cause it to omit lense nonshapelet sources here, since they'll be included in the foreground SB calculation
					image_pixel_grid->PSF_convolution_pixel_vector_wrapper<PlainTypes>(false,verbose,fft_convolution);
					image_pixel_grid->store_image_pixel_surface_brightness<PlainTypes>();
				} else {
					//image_pixel_grid->find_surface_brightness(false,fg_only,lensed_only,include_potential_perturbations and first_order_sb_correction,show_only_first_order_corrections,include_noninverted_src_as_foreground); // the last argument will cause it to omit lense nonshapelet sources here, since they'll be included in the foreground SB calculation
					image_pixel_grid->find_surface_brightness_vec<PlainTypes>(false,fg_only,lensed_only,include_noninverted_src_as_foreground); // the last argument will cause it to omit lense nonshapelet sources here, since they'll be included in the foreground SB calculation
					//image_pixel_grid->vectorize_image_pixel_surface_brightness(); // note that in this case, the image pixel vector does NOT contain the foreground; the foreground PSF convolution was done separately above

					image_pixel_grid->PSF_convolution_pixel_vector_wrapper<PlainTypes>(false,verbose,fft_convolution);
					image_pixel_grid->store_image_pixel_surface_brightness<PlainTypes>();
				}
			} else {
				image_pixel_grid->set_zero_lensed_surface_brightness<double>();
			}
			...
			if ((n_ptsrc > 0) and (!exclude_ptimgs)) {
				if (zsrc_i==0) {
					...
					for (i=0; i < n_ptsrc; i++) {
						...
						if (!include_imgfluxes_in_inversion) image_pixel_grid->find_point_images(ptsrc_list[i]->ptsrc_params.pos[0],ptsrc_list[i]->ptsrc_params.pos[1],ptsrc_list[i]->images,false,is_lensed,verbose);
						image_pixel_grid->generate_and_add_point_images(ptsrc_list[i]->images, include_imgfluxes_in_inversion, ptsrc_list[i]->ptsrc_params.srcflux);
					}
				}
			}
	...
	if (use_data) {
		...
		for (zsrc_i=zsrc_i_0, imggrid_i=primary_imggrid_i; zsrc_i < zsrc_i_f; zsrc_i++, imggrid_i++) {
			...
			{
				if (!image_pixel_grids[imggrid_i]->set_fit_window((*image_data),true,assigned_mask[imggrid_i],true,include_fgmask_in_inversion)) {
...
bool QLens::set_shapelet_imgpixel_nsplit(const int imggrid_i)
{
	...
	if (image_pixel_grid == NULL) {
		image_pixel_grid = new ImagePixelGrid(this, source_fit_mode, (*imgdata_list[0]), include_fgmask_in_inversion, 0, imggrid_i, imggrid_i, assigned_mask[imggrid_i], true);
		if (n_psf > 0) psf_list[0]->set_image_pixel_grid(image_pixel_grids[imggrid_i]);
	}
	image_pixel_grid->redo_lensing_calculations<PlainTypes>();
...
void QLens::reset_PSF_convolution_plans()
{
	if (image_pixel_grids == NULL) return;
	for (int imggrid_i=0; imggrid_i < n_image_pixel_grids; imggrid_i++) {
		if (image_pixel_grids[imggrid_i] == NULL) continue;
		if ((image_pixel_grids[imggrid_i]->psf_convolution_is_setup) or (image_pixel_grids[imggrid_i]->fg_psf_convolution_is_setup)) image_pixel_grids[imggrid_i]->reset_psfconv_plans();
	}
}
...
bool QLens::load_pixel_grid_from_data(const int band_number)
{
	...
	for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
		...
		image_pixel_grids[imggrid_i] = new ImagePixelGrid(this, source_fit_mode, (*imgdata_list[band_number]), include_fgmask_in_inversion, band_number, zsrc_i, imggrid_i, assigned_mask[imggrid_i]);
...
void QLens::plot_image_pixel_grid(const int band_number, const int zsrc_i)
{
	...
	int imggrid_i = band_number*n_extended_src_redshifts + zsrc_i;
	if (image_pixel_grids[imggrid_i] == NULL) {
		image_pixel_grids[imggrid_i] = new ImagePixelGrid(this, source_fit_mode,(*imgdata_list[band_number]), include_fgmask_in_inversion, zsrc_i, imggrid_i, assigned_mask[imggrid_i]);
		if (band_number < n_psf) psf_list[band_number]->set_image_pixel_grid(image_pixel_grids[imggrid_i]);
	}
	image_pixel_grids[imggrid_i]->redo_lensing_calculations<PlainTypes>();
...
double QLens::invert_surface_brightness_map_from_data(double &chisq0, const bool verbal, const bool zero_verbal)
{
	...
	for (band_number=0; band_number < n_model_bands; band_number++) {
		...
		for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
			if (image_pixel_grids[imggrid_i] == NULL) {
				image_pixel_grids[imggrid_i] = new ImagePixelGrid(this, source_fit_mode, (*imgdata_list[band_number]), include_fgmask_in_inversion, band_number, zsrc_i, imggrid_i, assigned_mask[zsrc_i], true, verbal);
				if (band_number < n_psf) psf_list[band_number]->set_image_pixel_grid(image_pixel_grids[imggrid_i]);
			}
			imggrid_i++;
		}
	...
	chisq0=0;
	std::chrono::steady_clock::time_point inversion_wtime0;
	std::chrono::duration<double> inversion_wtime;
	if (show_wtime) {
		inversion_wtime0 = std::chrono::steady_clock::now();
	}
	for (int i=0; i < n_ranchisq; i++) {
	...
	if (show_wtime) {
		inversion_wtime = std::chrono::steady_clock::now() - inversion_wtime0;
		if (mpi_id==0) cout << "Total wall time for lensing reconstruction: " << inversion_wtime.count() << endl;
	}
...
double QLens::pixel_log_evidence_times_two(double &chisq0, const bool verbal, const int ranchisq_i)
{
	...
	std::chrono::steady_clock::time_point fspline_wtime0;
	std::chrono::duration<double> fspline_wtime;
	if (show_wtime) {
		fspline_wtime0 = std::chrono::steady_clock::now();
	}
	...
	if ((show_wtime) and (splined_fourier_integrals)) {
		fspline_wtime = std::chrono::steady_clock::now() - fspline_wtime0;
		if (mpi_id==0) cout << "Wall time for splining Fourier integrals: " << fspline_wtime.count() << endl;
	}

	if ((redo_lensing_calculations_before_inversion) and (ranchisq_i==0)) {
		for (imggrid_i=0; imggrid_i < n_image_pixel_grids; imggrid_i++) {
			image_pixel_grids[imggrid_i]->redo_lensing_calculations<PlainTypes>(verbal);
		}
	}
	for (imggrid_i=0; imggrid_i < n_image_pixel_grids; imggrid_i++) {
	...
	if ((n_image_prior) or (n_ptsrc > 0)) {
		setup_auxiliary_sourcegrids_and_point_imgs<PlainTypes>(src_i_list,verbal);
	}
	...
	for (int band_number = 0; band_number < n_data_bands; band_number++) {
		...
		if (source_fit_mode == Cartesian_Source) {
			for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
				...
				if (src_i < 0) {
					...
					image_pixel_grid->vectorize_image_pixel_surface_brightness();
					...
					if (!setup_cartesian_sourcegrid(imggrid_i,src_i,n_expected_imgpixels,verbal)) { delete[] src_i_list; return 2e30; }
					...
					if (adaptive_subgrid) {
						...
						if (n_image_prior) cartesian_srcgrids[src_i]->calculate_pixel_magnifications<PlainTypes>();
					}

					if (show_wtime) {
						srcgrid_wtime = std::chrono::steady_clock::now() - srcgrid_wtime0;
						if (mpi_id==0) cout << "Wall time for creating source pixel grid: " << srcgrid_wtime.count() << endl;
					}
					if (!generate_and_invert_lensing_matrix_cartesian(imggrid_i,src_i,tot_wtime,tot_wtime0,verbal)) { delete[] src_i_list; return 2e30; }

					if (matrix_format==DENSE) image_pixel_grid->calculate_image_pixel_surface_brightness_dense<PlainTypes>();
			...
			for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
				...
				if (src_i >= 0) {
					...
					if (nlens > 0) {
						...
						create_sourcegrid_from_imggrid_delaunay<PlainTypes>(use_weighted_clustering,band_number,zsrc_i,verbal);
						...
						if ((include_potential_perturbations) and (imggrid_i==0)) {
							if (create_lensgrid_cartesian(band_number,zsrc_i,0,verbal)==false) { delete[] src_i_list; return 2e30; }
							lensgrids[0]->include_in_lensing_calculations = false;
							if (show_wtime) {
								srcgrid_wtime = std::chrono::steady_clock::now() - srcgrid_wtime0;
								if (mpi_id==0) cout << "wall time for pixellated potential grid creation: " << srcgrid_wtime.count() << endl;
							}
						}
					}
				}
			...
			for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
				...
				if (src_i < 0) {
					...
					if ((!use_lum_weighted_srcpixel_clustering) and (!use_saved_sbweights) and (save_sbweights_during_inversion)) image_pixel_grid->calculate_subpixel_sbweights(true,verbal);
			...
			for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
				...
				if (at_least_one_lensed_src) {
					image_pixel_grid->find_surface_brightness_vec<PlainTypes>(false,false,true);
					//image_pixel_grid->vectorize_image_pixel_surface_brightness();
					image_pixel_grid->PSF_convolution_pixel_vector_wrapper<PlainTypes>(false,verbal,fft_convolution);
					image_pixel_grid->store_image_pixel_surface_brightness<PlainTypes>();
				} else {
					image_pixel_grid->set_zero_lensed_surface_brightness<double>();
				}
			...
			for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
				...
				if (!skip_inversion) {
					...
					image_pixel_grid->set_surface_brightness_vector_to_data(); // note that image_pixel_grids[0] just has the data pixel values stored in it
					image_pixel_grid->PSF_convolution_Lmatrix_dense_wrapper<PlainTypes>(verbal);
					if (imggrid_i==0) {
						// currently only allowing point sources with first image grid...will extend later
						if ((n_ptsrc > 0) and (!include_imgfluxes_in_inversion) and (!include_srcflux_in_inversion)) {
							if ((mpi_id==0) and (verbal)) cout << "Generating point images..." << endl;
							for (i=0; i < n_ptsrc; i++) {
								image_pixel_grid->generate_point_images(ptsrc_list[i]->images, imggrid.point_image_surface_brightness, false, ptsrc_list[i]->get_srcflux());
							}
						}
					}
					...
					if ((optimize_regparam) and (regularization_method != None) and (image_pixel_grid->source_npixels > 0)) image_pixel_grid->optimize_regularization_parameter<PlainTypes>(true,verbal);
					if ((!optimize_regparam) or (image_pixel_grid->source_npixels==0) or (regularization_method==None)) image_pixel_grid->invert_lens_mapping_dense(verbal); 
					if (save_sbweights_during_inversion) image_pixel_grid->calculate_subpixel_sbweights(true,verbal); // these are sb-weights to be used later in Delaunay mode for luminosity weighting
					image_pixel_grid->calculate_image_pixel_surface_brightness_dense<PlainTypes>();
					image_pixel_grid->store_image_pixel_surface_brightness<PlainTypes>();
					if ((n_ptsrc > 0) and (!include_imgfluxes_in_inversion) and (!include_srcflux_in_inversion)) {
						image_pixel_grid->add_point_images(imggrid.point_image_surface_brightness,image_pixel_grid->n_active_pixels);
					}
		...
		for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
			...
			//ImgGrid_Params<double>& imggrid_params = image_pixel_grid->assign_imggrid_param_object<double>();
			n_data_pixels = 0;
			chisq0_imggrid = 0;
			...
			if (include_noise_term_in_loglike) {
				...
				if (use_noise_map) {
					bool include_pixel;
					for (int img_index=0; img_index < image_pixel_grid->image_npixels; img_index++) {
						include_pixel = true;
						i = image_pixel_grid->emask_pixels_i[img_index];
						j = image_pixel_grid->emask_pixels_j[img_index];
			...
			if (src_i_list[imggrid_i] != -1) {
				if (n_image_prior) {
					if ((source_fit_mode == Cartesian_Source) or (source_fit_mode == Delaunay_Source) or ((source_fit_mode==Shapelet_Source) and (at_least_one_shapelet_src))) {
						...
						if (source_fit_mode != Cartesian_Source) {
							if (source_fit_mode==Shapelet_Source) {
								image_pixel_grids[imggrid_i]->cartesian_srcgrid->assign_surface_brightness_from_analytic_source<double>(zsrc_i);
							} else if (source_fit_mode==Delaunay_Source) {
								image_pixel_grids[imggrid_i]->cartesian_srcgrid->assign_surface_brightness_from_delaunay_grid<double>(image_pixel_grids[imggrid_i]->delaunay_srcgrid);
							}
						}
...
QScalar QLens::pixel_log_evidence_times_two_delaunay(QScalar &chisq0, const bool verbal, const int ranchisq_i)
{
	...
	bool sb_outside_window, sb_outside_window_allbands = false;
	QScalar logev_penalty = 0, logev_penalty_band = 0;
	for (int band_number = 0; band_number < n_data_bands; band_number++) {
		image_data = imgdata_list[band_number];
		loglike_reg = 0;
		logev_times_two_band = 0;
		...
		for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
			...
			if (image_pixel_grids[imggrid_i]->n_pixsrc_to_include_in_Lmatrix==0) continue;
			ImgGrid_Params<MathTypes>& imggrid = image_pixel_grid->assign_imggrid_param_object<MathTypes>();
			//ImgGrid_Params<QScalar>& imggrid_params = image_pixel_grid->assign_imggrid_param_object<QScalar>();
			n_data_pixels = 0;
			chisq0_imggrid = 0;
			for (i=0; i < image_data->npixels_x; i++) {
				for (j=0; j < image_data->npixels_y; j++) {
			...
			if (src_i_list[imggrid_i] != -1) {
				if ((n_image_prior) and (source_fit_mode != Cartesian_Source) and (source_fit_mode != Parameterized_Source)) {
					...
					if ((source_fit_mode==Parameterized_Source) or (source_fit_mode==Shapelet_Source)) {
						image_pixel_grids[imggrid_i]->cartesian_srcgrid->assign_surface_brightness_from_analytic_source<QScalar>(zsrc_i);
					} else if (source_fit_mode==Delaunay_Source) {
						image_pixel_grids[imggrid_i]->cartesian_srcgrid->assign_surface_brightness_from_delaunay_grid<QScalar>(image_pixel_grids[imggrid_i]->delaunay_srcgrid);
					}
					pixel_avg_n_images = image_pixel_grids[imggrid_i]->cartesian_srcgrid->find_avg_n_images<QScalar>(n_image_prior_sb_frac);
					if (show_wtime) {
						wtime = std::chrono::steady_clock::now() - wtime0;
						if (mpi_id==0) cout << "Wall time for assigning SB for nimg_prior: " << wtime.count() << endl;
					}
...
//#endif
	...
	{
		if ((group_id==0) and (logfile.is_open())) {
			if (sb_outside_window_allbands) logfile << " -2*log(ev)=" << logev_times_two << " (without priors; SB produced outside window)" << endl;
			else logfile << " -2*log(ev)=" << logev_times_two << " (without priors)" << endl;
		}
		if ((mpi_id==0) and (verbal)) {
			cout << "-2*log(ev)=" << logev_times_two << " (a.k.a. 'chisq_pix')" << endl;
		}
		//if (sb_outside_window_allbands) cout << "outside_sb_penalty = " << logev_penalty << endl;
	}
	if ((!include_noise_term_in_loglike) and (regularization_method != None) and (source_fit_mode != Parameterized_Source)) {
		if ((mpi_id==0) and (verbal)) cout << "NOTE: the noise term(s) in the log(evidence) are NOT being included (to include, set 'include_noise_term_in_loglike' to 'on')" << endl;
	}
...
template <typename QScalar, typename MathTypes>
QScalar QLens::pixel_log_evidence_times_two_sbprofile(QScalar &chisq0, const bool verbal)
{
	if (n_data_bands==0) { warn("No image data have been loaded"); return -1e30; }
	if (n_model_bands < n_data_bands) { warn("Numebr of model bands is not large enough to accommodate number of data bands"); }
	...
	for (int band_number = 0; band_number < n_data_bands; band_number++) {
		...
		for (zsrc_i=0, imggrid_i=band_number*n_extended_src_redshifts; zsrc_i < n_extended_src_redshifts; zsrc_i++, imggrid_i++) {
			if (at_least_one_lensed_src) {
				image_pixel_grids[imggrid_i]->find_surface_brightness_vec<MathTypes>(false,false,true,false);
				image_pixel_grids[imggrid_i]->PSF_convolution_pixel_vector_wrapper<MathTypes>(false,verbal,fft_convolution);
				image_pixel_grids[imggrid_i]->store_image_pixel_surface_brightness<MathTypes>();
			} else {
				image_pixel_grids[imggrid_i]->set_zero_lensed_surface_brightness<QScalar>();
			}
			//if (at_least_one_noninverted_foreground_src) {
				//assign_foreground_mappings(imggrid_i,true);
				//if (!ignore_foreground_in_chisq) {
					//calculate_foreground_pixel_surface_brightness(imggrid_i,false);
					//image_pixel_grids[imggrid_i]->store_foreground_pixel_surface_brightness();
				//}
			//} else {
				//image_pixel_grids[imggrid_i]->set_zero_foreground_surface_brightness<QScalar>();
			//}
		}
		...
		if constexpr (std::is_same_v<QScalar, stan::math::var>)
		{
			...
			ImgGrid_Params<Eigen::VectorXd,Eigen::MatrixXd,double>& imggrid_params_nodif = image_pixel_grid->imggrid_params;
	...
	if (show_wtime) {
		tot_wtime = std::chrono::steady_clock::now() - tot_wtime0;
		if (mpi_id==0) cout << "Total wall time for sbprofile loglike: " << tot_wtime.count() << endl;
	}
...
void QLens::setup_auxiliary_sourcegrids_and_point_imgs(int* src_i_list, const bool verbal)
{
	...
	for (int imggrid_i=0; imggrid_i < n_image_pixel_grids; imggrid_i++) {
		...
		if (source_fit_mode != Cartesian_Source) {
			...
			if (nlens > 0) {
				...
				if ((source_fit_mode==Shapelet_Source) and (n_image_prior)) { // note, in shapelet mode, we only need the auxiliary grid if using n_image_prior
					create_sourcegrid_cartesian<MathTypes>(band_number,zsrc_i,verbal,true,false,true,true);
			...
			if (source_grid_defined) {
				if (show_wtime) {
					srcgrid_wtime = std::chrono::steady_clock::now() - srcgrid_wtime0;
					if (mpi_id==0) cout << "wall time for auxiliary source grid creation: " << srcgrid_wtime.count() << endl;
				}
			}
		} else {
			source_grid_defined = true;
		...
		if (source_grid_defined) {
			image_pixel_grids[imggrid_i]->set_cartesian_srcgrid(cartesian_srcgrids[src_i]);
			cartesian_srcgrids[src_i]->set_image_pixel_grid(image_pixel_grids[imggrid_i]);
			if (!adaptive_subgrid) cartesian_srcgrids[src_i]->calculate_pixel_magnifications<MathTypes>(); // if adaptive_subgrid is off, we still need to get pixel magnifications for nimg_prior
		}
		if (imggrid_i==0) {
			if (n_ptsrc > 0) {
				...
				for (i=0; i < n_ptsrc; i++) {
					...
					image_pixel_grids[imggrid_i]->find_point_images(ptsrc_list[i]->ptsrc_params.pos[0],ptsrc_list[i]->ptsrc_params.pos[1],ptsrc_list[i]->images,source_grid_defined,is_lensed,verbal);
				}
			}
			for (i=0; i < n_sb; i++) {
...
template void QLens::setup_auxiliary_sourcegrids_and_point_imgs<PlainTypes>(int* src_i_list, const bool verbal);
#ifdef USE_STAN
template void QLens::setup_auxiliary_sourcegrids_and_point_imgs<VarmatTypes>(int* src_i_list, const bool verbal);
#endif

bool QLens::setup_cartesian_sourcegrid(const int imggrid_i, const int src_i, int& n_expected_imgpixels, const bool verbal)
{
	image_pixel_grids[imggrid_i]->set_cartesian_srcgrid(cartesian_srcgrids[src_i]);
	if (auto_sourcegrid) image_pixel_grids[imggrid_i]->find_optimal_sourcegrid<double,PlainTypes>(sourcegrid_xmin,sourcegrid_xmax,sourcegrid_ymin,sourcegrid_ymax,sourcegrid_limit_xmin,sourcegrid_limit_xmax,sourcegrid_limit_ymin,sourcegrid_limit_ymax);
	if (auto_srcgrid_npixels) {
	...
	if (auto_sourcegrid) {
		if (image_pixel_grids[imggrid_i]->cartesian_srcgrid->cartesian_srcgrid_params.srcgrid_size_scale != 0) {
			double xwidth_adj = image_pixel_grids[imggrid_i]->cartesian_srcgrid->cartesian_srcgrid_params.srcgrid_size_scale*(sourcegrid_xmax-sourcegrid_xmin);
			double ywidth_adj = image_pixel_grids[imggrid_i]->cartesian_srcgrid->cartesian_srcgrid_params.srcgrid_size_scale*(sourcegrid_ymax-sourcegrid_ymin);
...
bool QLens::generate_and_invert_lensing_matrix_cartesian(const int imggrid_i, const int src_i, std::chrono::duration<double>& tot_wtime, const std::chrono::steady_clock::time_point& tot_wtime0, const bool verbal)
{
	...
	double est_pixfrac = est_nmapped / image_pixel_grid->image_npixels;
	if ((mpi_id==0) and (verbal)) {
		double pixfrac = ((double) image_pixel_grid->n_amps) / image_pixel_grid->image_npixels;
		cout << "Actual f = " << pixfrac << endl;
		if (auto_srcgrid_npixels) {
			double high_sn_pixfrac = ((double) image_pixel_grid->n_amps*high_sn_srcgrid_overlap_area/total_srcgrid_overlap_area) / image_pixel_grid->n_high_sn_pixels;
			cout << "Actual high S/N f = " << high_sn_pixfrac << endl;
		}
	}
	if ((mpi_id==0) and (verbal)) cout << "Initializing pixel matrices...\n";
	image_pixel_grid->initialize_pixel_matrices<PlainTypes>(false,verbal);
	if (regularization_method != None) image_pixel_grid->create_regularization_matrix<PlainTypes>();
	if (matrix_format==DENSE) {
		image_pixel_grid->PSF_convolution_Lmatrix_dense_wrapper<PlainTypes>(verbal);
	} else {
		image_pixel_grid->PSF_convolution_Lmatrix(verbal);
	...
	if ((n_ptsrc > 0) and (!include_imgfluxes_in_inversion) and (!include_srcflux_in_inversion)) {
		if ((mpi_id==0) and (verbal)) cout << "Generating point images..." << endl;
		for (int i=0; i < n_ptsrc; i++) {
			image_pixel_grid->generate_point_images(ptsrc_list[i]->images, imggrid.point_image_surface_brightness, include_imgfluxes_in_inversion, ptsrc_list[i]->get_srcflux());
		}
	}

	if ((mpi_id==0) and (verbal)) cout << "Creating lensing matrices...\n" << flush;
	bool dense_Fmatrix = ((matrix_format==DENSE) or (matrix_format==DENSE_FMATRIX)) ? true : false;
	if (matrix_format==DENSE) image_pixel_grid->create_lensing_matrices_from_Lmatrix_dense<PlainTypes>(false,false,verbal);
	else image_pixel_grid->create_lensing_matrices_from_Lmatrix(dense_Fmatrix,false,verbal);
	if (show_wtime) {
		tot_wtime = std::chrono::steady_clock::now() - tot_wtime0;
		if (mpi_id==0) cout << "Total wall time before F-matrix inversion: " << tot_wtime.count() << endl;
	}
	...
	if ((optimize_regparam) and (regularization_method != None)) {
		image_pixel_grid->optimize_regularization_parameter<PlainTypes>(dense_Fmatrix,verbal);
	}
	if (!optimize_regparam) {
		if ((matrix_format==DENSE) or (matrix_format==DENSE_FMATRIX)) image_pixel_grid->invert_lens_mapping_dense(verbal);
		else {
			if (sparse_solver==MUMPS) image_pixel_grid->invert_lens_mapping_MUMPS(image_pixel_grid->imggrid_params.Fmatrix_log_determinant,verbal);
			else if (sparse_solver==UMFPACK) image_pixel_grid->invert_lens_mapping_UMFPACK(image_pixel_grid->imggrid_params.Fmatrix_log_determinant,verbal);
			//else if (sparse_solver==EIGEN_SPARSE) image_pixel_grid->invert_lens_mapping_EIGEN_sparse(image_pixel_grid->imggrid_params.Fmatrix_log_determinant,verbal);
			//else if (sparse_solver==CG_Method) image_pixel_grid->invert_lens_mapping_CG_method(verbal);
			else {
...
bool QLens::generate_and_invert_lensing_matrix_delaunay(const int imggrid_i, const int src_i, const bool potential_perturbations, const bool save_sb_gradient, std::chrono::duration<double>& tot_wtime, const std::chrono::steady_clock::time_point& tot_wtime0, const bool verbal)
{
	...
	if ((mpi_id==0) and (verbal)) cout << "Initializing pixel matrices...\n";
	image_pixel_grid->initialize_pixel_matrices<MathTypes>(potential_perturbations,verbal);
	...
	if ((use_lum_weighted_srcpixel_clustering) and (!use_saved_sbweights)) {
		...
		create_sourcegrid_from_imggrid_delaunay<MathTypes>(true,image_pixel_grid->band_number,image_pixel_grid->src_redshift_index,verbal);

		if (show_wtime) {
			srcgrid_wtime = std::chrono::steady_clock::now() - srcgrid_wtime0;
			if (mpi_id==0) cout << "wall time for Delaunay grid creation (with lum weighting): " << srcgrid_wtime.count() << endl;
		}
		image_pixel_grid->set_delaunay_srcgrid(delaunay_srcgrids[src_i]);
		...
		if ((mpi_id==0) and (verbal)) {
			cout << "Number of active image pixels (with lum weighting): " << image_pixel_grid->image_npixels << endl;
		}
		...
		if (show_wtime) {
			tot_wtime = std::chrono::steady_clock::now() - tot_wtime0;
			if (mpi_id==0) cout << "Total wall time before F-matrix inversion (with lum weighting): " << tot_wtime.count() << endl;
		}
		if ((mpi_id==0) and (verbal)) cout << "Inverting lens mapping...\n" << flush;
		if ((optimize_regparam) and (regularization_method != None) and (!potential_perturbations) and (image_pixel_grid->delaunay_srcgrid != NULL)) {
	...
	if ((!optimize_regparam) or (using_autodiff) or (potential_perturbations)) {
		...
		if ((optimize_regparam) and (using_autodiff) and (matrix_format==DENSE)) {
			image_pixel_grid->create_lensing_matrices_from_Lmatrix_dense<MathTypes>(potential_perturbations,true,verbal); // just adding the regularization term here
		}
		if ((matrix_format==DENSE) or (matrix_format==DENSE_FMATRIX)) {
#ifdef USE_STAN
	...
	if (save_sb_gradient) image_pixel_grid->calculate_subpixel_source_gradient();

	return true;
}
template bool QLens::generate_and_invert_lensing_matrix_delaunay<PlainTypes>(const int imggrid_i, const int src_i, const bool potential_perturbations, const bool save_sb_gradient, std::chrono::duration<double>& tot_wtime, const std::chrono::steady_clock::time_point& tot_wtime0, const bool verbal);
#ifdef USE_STAN
...
typename MathTypes::QScalar QLens::find_outside_sb_prior_penalty(const int band_number, int* src_i_list, bool& sb_outside_window, const bool verbal)
{
	...
	if ((source_fit_mode==Cartesian_Source) or (source_fit_mode==Delaunay_Source)) {
		...
		std::chrono::steady_clock::time_point sbwtime0;
		std::chrono::duration<double> sbwtime;
		if (show_wtime) {
			sbwtime0 = std::chrono::steady_clock::now();
		}
		for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
			imggrid_i = band_number*n_extended_src_redshifts + zsrc_i;
			ImagePixelGrid* image_pixel_grid = image_pixel_grids[imggrid_i]; 
			image_pixel_grid->find_surface_brightness_vec<MathTypes>(true,false,true);
			//image_pixel_grid->vectorize_image_pixel_surface_brightness(true);
			if (show_wtime) {
				sbwtime = std::chrono::steady_clock::now() - sbwtime0;
				if (mpi_id==0) cout << "Wall time for calculating SB outside mask: " << sbwtime.count() << endl;
			}
			image_pixel_grid->PSF_convolution_pixel_vector_wrapper<MathTypes>(false,verbal,false,true); // no supersampling, no fft convolution (saves time)
			image_pixel_grid->store_image_pixel_surface_brightness<MathTypes>(true);
		}
	} else if (source_fit_mode==Parameterized_Source) {
		for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
			...
			image_pixel_grid->find_surface_brightness_vec<MathTypes>(true,false,true);
			//image_pixel_grid->vectorize_image_pixel_surface_brightness(true);
			image_pixel_grid->PSF_convolution_pixel_vector_wrapper<MathTypes>(false,verbal,false,true); // no supersampling, no convolution (saves time)
			image_pixel_grid->store_image_pixel_surface_brightness<MathTypes>(true);
		}
	}
	...
	for (zsrc_i=0; zsrc_i < n_extended_src_redshifts; zsrc_i++) {
		...
		double max_external_sb_doub = -1e30, max_sb_doub = -1e30;
		for (i=0; i < image_data->npixels_x; i++) {
			for (j=0; j < image_data->npixels_y; j++) {
		...
		bool used_noise_threshold = false;
		double outside_sb_threshold;
		double noise_threshold_try, sb_threshold_try;
		noise_threshold_try = outside_sb_prior_noise_frac*bg_noise;
		sb_threshold_try = outside_sb_prior_threshold*max_sb_doub;
		if (sb_threshold_try > noise_threshold_try) outside_sb_threshold = sb_threshold_try;
		else {
			used_noise_threshold = true;
			outside_sb_threshold = noise_threshold_try;
		}
		dmax(outside_sb_prior_noise_frac*bg_noise,outside_sb_prior_threshold*max_sb_doub);
		int isb, jsb;
			...
			for (j=0; j < image_data->npixels_y; j++) {
				if ((!mask_for_inversion[i][j]) and ((image_pixel_grids[imggrid_i]->emask[i][j])) and (image_pixel_grids[imggrid_i]->maps_to_source_pixel[i][j])) {
					...
					if (abs(image_pixel_grids[imggrid_i]->surface_brightness[i][j]) >= outside_sb_threshold) {
						if (image_pixel_grids[imggrid_i]->surface_brightness[i][j] > max_external_sb_doub) {
							 max_external_sb_doub = abs(image_pixel_grids[imggrid_i]->surface_brightness[i][j]);
							 isb=i; jsb=j;
						}
					}
				}
		...
		//cout << "max_external_sb: " << max_external_sb_doub << endl;
		//cout << "max_sb_doub: " << max_sb_doub << " isb=" << isb << " jsb=" << jsb << " i_maxsb=" << i_maxsb << " j_maxsb=" << j_maxsb << endl;
		if (max_external_sb_doub > 0) {
			QScalar max_external_sb, max_sb;
			sb_outside_window = true;
			ImgGrid_Params<MathTypes>& imggrid_params = image_pixel_grids[imggrid_i]->assign_imggrid_param_object<MathTypes>();
			int idx = image_pixel_grids[imggrid_i]->pixel_index[isb][jsb];
			max_external_sb = abs(imggrid_params.image_surface_brightness_emask(idx));
			QScalar outside_sb_threshold;
			if (used_noise_threshold) outside_sb_threshold = outside_sb_prior_noise_frac*bg_noise;
			else {
				idx = image_pixel_grids[imggrid_i]->pixel_index[i_maxsb][j_maxsb];
