...
PYBIND11_MODULE(qlens, m) {
	...
	*/
	...
	py::class_<QLens_Wrap>(m, "QLens")
		...
		.def("output_coolest", [](QLens_Wrap &curr, std::string filename){
			if (LensProfile::use_ellipticity_components) throw std::runtime_error("ellipticity components must be turned off before generating COOLEST json file");
			if (Shear::use_shear_component_params) throw std::runtime_error("shear components must be turned off before generating COOLEST json file");
			if (!curr.output_coolest_files(filename)) throw std::runtime_error("could not output coolest .json file");
		...
		.def("fit_chisq",&QLens_Wrap::chisq_single_evaluation, py::arg("init_fitmodel") = false, py::arg("show_total_wtime") = false, py::arg("show_wtime") = false, py::arg("showdiag") = false, py::arg("show_status") = true, py::arg("show_lensmodel") = false, py::arg("grad") = false, py::arg("testgrad") = false)
