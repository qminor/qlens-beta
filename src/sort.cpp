...
#define SWAPDOUB(a,b) temp=(a);(a)=(b);(b)=temp_doub;
...
template <typename QScalar>
void Sort::sort(const int n, QScalar arr[], QScalar brr[], QScalar crr[]) 
{
	const int M = 7;
	const int nstack = 50;
...
template <typename QScalar>
void Sort::sort_mixedtype(const int n, double arr[], QScalar brr[], QScalar crr[]) 
{
	const int M = 7;
	const int nstack = 50;
	...
	for (;;) {
		if (ir-l < M) {
			...
			k = (l+ir) >> 1;
			SWAPDOUB(arr[k],arr[l+1])
			SWAP(brr[k],brr[l+1])
			SWAP(crr[k],crr[l+1])
			if (arr[l] > arr[ir]) {
				SWAPDOUB(arr[l],arr[ir])
				SWAP(brr[l],brr[ir])
				SWAP(crr[l],crr[ir])
			}
			if (arr[l+1] > arr[ir]) {
				SWAPDOUB(arr[l+1],arr[ir])
				SWAP(brr[l+1],brr[ir])
				SWAP(crr[l+1],crr[ir])
			}
			if (arr[l] > arr[l+1]) {
				SWAPDOUB(arr[l],arr[l+1])
				SWAP(brr[l],brr[l+1])
				SWAP(crr[l],crr[l+1])
			}
			i = l + 1;
...
template void Sort::sort_mixedtype<double>(const int n, double arr[], double brr[], double crr[]);
#ifdef USE_STAN
template void Sort::sort_mixedtype<stan::math::var>(const int n, double arr[], stan::math::var brr[], stan::math::var crr[]);
#endif
