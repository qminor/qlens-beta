...
{
	return lens->inverse_magnification(ccsearch_initial_pt + t*ccsearch_interval,0,zfactor);
}
...
void Lens::output_images_single_source(const double &x_source, const double &y_source, bool verbal, const double flux, const bool show_labels)
{
	...
	if (grid==NULL) {
		if (create_grid(verbal,reference_zfactor)==false) return;
	}
...
bool Lens::plot_images_single_source(const double &x_source, const double &y_source, bool verbal, string imgfilename, string srcfilename, const double flux, const bool show_labels)
{
	...
	if ((use_cc_spline) and (!cc_splined) and (spline_critical_curves()==false)) return false;
	if ((grid==NULL) and (create_grid(verbal,reference_zfactor)==false)) return false;
...
image* Lens::get_images(const lensvector &source_in, int &n_images, bool verbal)
{
	...
	if (grid==NULL) {
		if (create_grid(verbal,reference_zfactor)==false) return NULL;
	}
...
inline void Lens::lens_equation(const lensvector& x, lensvector& f, const int& thread, const double zfac)
{
	deflection(x[0],x[1],f,thread,zfac);
...
bool Grid::run_newton(const lensvector& xroot_initial, const int& thread)
{
	...
	#pragma omp critical
	{
		if (redundancy(xroot)) {
			...
			images[nfound].mag = lens->magnification(xroot,0,zfactor);
			if (lens->include_time_delays) {
				double potential = lens->potential(xroot,zfactor);
...
bool Grid::NewtonsMethod(lensvector& x, bool &check, const int& thread)
{
	...
	for (int its=0; its < max_iterations; its++) {
		lens->hessian(x[0],x[1],fjac,thread,zfactor);
		fjac[0][0] = -1 + fjac[0][0];
		fjac[1][1] = -1 + fjac[1][1];
