...
q.nrepeat = 2
print("Source model:",ptsrc,"\n")
#q.fitmodel()
print("Fit parameters:",params,"\n")
pause() # note, pause will be ignored if script is not run in interactive mode (with '-i' parameter)

# If you want to evaluate the likelihood from Python to optimize from the Python side), the following lines must be run first
...
#logl = q.LogLike(params.values())  # Now you can evaluate the likelihood
