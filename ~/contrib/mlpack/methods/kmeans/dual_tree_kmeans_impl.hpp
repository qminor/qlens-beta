...
TreeType* BuildForcedLeafSizeTree(
    MatType&& dataset, // 'dataset' here is the centroids
...
DualTreeKMeans<MetricType, MatType, TreeType>::DualTreeKMeans(
    ...
    tree(new Tree(const_cast<MatType&>(dataset),20,&weights)),
...
void DualTreeKMeans<MetricType, MatType, TreeType>::ExtractCentroids(
    ...
    const arma::mat& centroids)
{
    if ((include_weights) && (node.NumChildren() == 0))
    {
		double w, tw=0;
      for (size_t i = 0; i < node.NumPoints(); ++i)
      {
      ...
      (node.Stat().StaticPruned() && node.Stat().Owner() < newCentroids.n_cols))
  {
    const size_t owner = node.Stat().Owner();
	 if (include_weights) {
			 //if (node.Stat().TotWeight() < 0.0) std::cout << "FUCK! totweight never initialized" << endl;
			 //std::cout << "totweight: " << node.Stat().TotWeight() << endl;
		 newCentroids.col(owner) += node.Stat().Centroid() * node.Stat().TotWeight();
		 newCounts[owner] += node.Stat().TotWeight();
		 //if (node.Stat().TotWeight() != (double) node.NumDescendants()) std::cout << "OOPS (extract_centroids), totweight != NumDescendants() (" << node.Stat().TotWeight() << " vs. " << (node.NumDescendants()) << " (numchildren=" << node.NumChildren() << ", numpoints=" << node.NumPoints() << ")" << endl;
		 //else std::cout << "totweight looks good!" << endl;
	 } else {
