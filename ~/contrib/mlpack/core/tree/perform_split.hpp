...
size_t PerformSplit(MatType& data,
                    const size_t begin,
                    const size_t count,
                    const typename SplitType::SplitInfo& splitInfo, arma::Col<double>* weights = NULL)
{
  ...
  while (left <= right)
  {
    ...
    data.swap_cols(left, right);
	 if (weights != NULL) weights->swap_rows(left, right);
	 //std::cout << "Swapping happened (" << left << " and " << right << ")" << endl;
...
size_t PerformSplit(MatType& data,
                    ...
                    std::vector<size_t>& oldFromNew, arma::Col<double>* weights = NULL)
{
  // This method modifies the input dataset.  We loop both from the left and
  // right sides of the points contained in this node.
  size_t left = begin;
