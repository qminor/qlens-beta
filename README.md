...
# change log (Mar. 18, 2018)

Upgrades since Feb. 9:

1. There is now a command, 'mass\_r' that allows you to find the 2D mass and 3D mass enclosed within a specified radius. For example, if you want the mass within 20 arcseconds for lens #0, you type "mass\_r 0 20". Qlens will then spit out the 2D and 3D masses enclosed as well as the 3D density at that radius, and will also spit out your radius in kpc along with the masses. Also, when using the "cosmology" command, qlens will spit out the total mass if the lens has a well-defined total mass, as well as the half-mass radius. If you don't see these values, it is because the total mass did not converge and is therefore infinite for that particular lens model--this is true for the NFW model, for example. All these quantities are calculated by numerical integration, although I may later override them with analytic formulas in some cases. All masses are calculated for the corresponding spherical profile, and hence have no dependence on the axis ratio q (another way to think of it is the mass within a given elliptical radius, defined by the 'emode' in qlens). Both the 2d and 3d masses are now available as derived parameters (type "help fit dparams" for usage).
