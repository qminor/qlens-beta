...
1. QLens can now be compiled with the PolyChord and/or MultiNest packages and used for nested sampling. To do this, install either package, and uncomment the relevant lines in the makefile. After compiling, successfully, you'll also need to make sure the directory containing the shared library is in your LD\_LIBRARY\_PATH environment variable before running qlens. Then, just change the fit method to "multinest" or "polychord", and you're all set. MPI parallelism can be used, but the only feature that isn't working with these packages is the ability to have multiple MPI groups (using the '-g' argument), such that each likelihood evaluation is done by a subgroup of MPI processes; hopefully later I'll get this working (although it might requrie modifications to the Fortran code in either package...ugh). However, you can still use OpenMP to multithread the likelihood in any case.
